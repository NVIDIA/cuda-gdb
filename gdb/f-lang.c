			...
			int *pos, int nargs, enum noside noside)
{
  ...
  std::vector<struct type *> dim_types;
  {
    ...
    for (int i = 0; i < ndimensions; ++i)
      {
#ifdef NVIDIA_CUDA_GDB
	/* CUDA: The subrange bounds maybe location expressions. We want to
	 * resolve those now. */
  ...
  for (int i = 0; i < nargs; ++i)
      ...
      else
	{
	  ...
	  if (index < lb
		...
		error (_("no such vector element (vector not allocated)"));
#ifdef NVIDIA_CUDA_GDB
	      /* CUDA: Elide bounds check on final dim to allow assumed array printing. 
	       *       This isn't safe since we could print values off the end of the array
	       *       but the alternative is the user is unable to print array elements. */
	      else if (i < nargs-1)
#else
	      else
#endif
...
#ifdef NVIDIA_CUDA_GDB
/* Auto unpack pointers.  This is improves the usefulness of pointer
   variables printed as part of backtraces and info locals as the
   content of the pointee is shown.
*/
struct value *
f_follow_pointers (struct value *v)
{
  ...
  while (type->code () == TYPE_CODE_PTR)
    {
      CORE_ADDR address = unpack_pointer (type, value_contents (v));
