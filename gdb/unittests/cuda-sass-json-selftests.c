...
{
public:
  static const char *sass_cuda_13_2_json;
...
newlines"
  ...
  _verify_cuda_sass_json_parse_sample_schema (schema_json schema)
  {
    ...
    SELF_CHECK (schema.m_metadata.m_schema_ver_major == 13);
    SELF_CHECK (schema.m_metadata.m_schema_ver_minor == 1);
    SELF_CHECK (schema.m_metadata.m_schema_ver_revision == 0);
    SELF_CHECK (schema.m_metadata.m_sm_ver_major == 10);
    SELF_CHECK (schema.m_metadata.m_sm_ver_minor == 0);
    SELF_CHECK (schema.m_metadata.m_producer == "nvdisasm V13.2.0 Build");
    SELF_CHECK (schema.m_metadata.m_description == "");
    ...
    SELF_CHECK (schema.m_functions[0].m_instructions.size () == 6);
    ...
    SELF_CHECK (schema.m_functions[0].m_instructions[1].m_predicate == "@P1");
    SELF_CHECK (schema.m_functions[0].m_instructions[1].m_opcode
		== "BPT.TRAP");
    SELF_CHECK (schema.m_functions[0].m_instructions[1].m_operands == "0x1");
    SELF_CHECK (schema.m_functions[0].m_instructions[1].m_extra == "");
    SELF_CHECK (
	schema.m_functions[0].m_instructions[1].m_other_attributes.size ()
	== 2);
    SELF_CHECK (
	schema.m_functions[0].m_instructions[1].m_other_attributes[0].first
	== "control-flow");
    SELF_CHECK (
	schema.m_functions[0].m_instructions[1].m_other_attributes[0].second
	== "True");
    SELF_CHECK (
	schema.m_functions[0].m_instructions[1].m_other_attributes[1].first
	== "another-attribute");
    SELF_CHECK (
	schema.m_functions[0].m_instructions[1].m_other_attributes[1].second
	== "some value");
    SELF_CHECK (schema.m_functions[0].m_instructions[1].m_other_flags.size ()
		== 1);
    SELF_CHECK (schema.m_functions[0].m_instructions[1].m_other_flags[0]
		== "another-flag2");
    ...
    SELF_CHECK (
	schema.m_functions[0].m_instructions[2].m_other_attributes.size ()
	== 1);
    SELF_CHECK (
	schema.m_functions[0].m_instructions[2].m_other_attributes[0].first
	== "control-flow");
    SELF_CHECK (
	schema.m_functions[0].m_instructions[2].m_other_attributes[0].second
	== "True");

    SELF_CHECK (schema.m_functions[0].m_instructions[3].m_predicate == "");
    SELF_CHECK (schema.m_functions[0].m_instructions[3].m_opcode
		== "BAR.SYNC.DEFER_BLOCKING");
    SELF_CHECK (schema.m_functions[0].m_instructions[3].m_operands == "0x0");
    SELF_CHECK (schema.m_functions[0].m_instructions[3].m_extra == "");
    SELF_CHECK (
	schema.m_functions[0].m_instructions[3].m_other_attributes.size ()
	== 1);
    SELF_CHECK (
	schema.m_functions[0].m_instructions[3].m_other_attributes[0].first
	== "inter-warp-barrier");
    SELF_CHECK (
	schema.m_functions[0].m_instructions[3].m_other_attributes[0].second
	== "True");

    SELF_CHECK (schema.m_functions[0].m_instructions[4].m_predicate == "");
    SELF_CHECK (schema.m_functions[0].m_instructions[4].m_opcode == "NOP");
    SELF_CHECK (schema.m_functions[0].m_instructions[4].m_operands == "");
    SELF_CHECK (schema.m_functions[0].m_instructions[4].m_extra == "extra");

    SELF_CHECK (schema.m_functions[0].m_instructions[5].m_predicate == "");
    SELF_CHECK (schema.m_functions[0].m_instructions[5].m_opcode == "");
    SELF_CHECK (schema.m_functions[0].m_instructions[5].m_operands == "");
    SELF_CHECK (schema.m_functions[0].m_instructions[5].m_extra == "");
  }

  static void
  test_cuda_sass_json_stream_parse ()
  {
    auto parser = make_parser (sass_cuda_13_2_json);
    schema_json schema;
  ...
  test_cuda_sass_json_fd_parse ()
  {
    ...
    SELF_CHECK (
	write (pipefd[1], sass_cuda_13_2_json, strlen (sass_cuda_13_2_json))
	== strlen (sass_cuda_13_2_json));
  ...
  test_cuda_sass_json_many_instructions ()
  {
    ...
    std::chrono::duration<float, std::milli> metadata_elapsed
	= metadata_time - start_time;
    ...
    std::cout << "Metadata parsing time: " << metadata_elapsed.count () << "ms"
	      << std::endl;
...
const char *selftests::cuda_sass_json::sass_cuda_13_2_json =
    R"(
[
	{
		"ELF": {
		...
		"Producer": "nvdisasm V13.2.0 Build",
		"Description": "",
		"additional-field": "this field is not part of the schema and should be ignored by the parser",
		".note.nv.cuinfo": {
			"nv_note_cuinfo": 2,
			"nv_note_cuinfo_virt_sm": 100,
			"nv_note_cuinfo_toolVersion": 132
		},
		".note.nv.tkinfo": {
			"nv_note_tkinfo": 2,
			"tki_objFname": "",
			"tki_toolName": "nvasm_internal",
			"tki_toolVersion": "Cuda compilation tools, release 13.2, V13.2.0",
			"tki_toolBranch": "Build",
			"tki_toolOptions": "-arch SM100 -arg1  -arg2  -arg3  "
		}
	},
	[
		{
			...
			"sass-instructions": [
				...
				{
					"opcode": "BAR.SYNC.DEFER_BLOCKING",
					"operands": "0x0",
					"other-attributes": {
						"inter-warp-barrier": "True"
					}
				},
				{
					"opcode": "NOP",
