			 ...
			 struct symtab_and_line val)
{
  ...
  gdb::unordered_set<struct symtab *> seen_symtab;
#endif
  std::vector<symtab_and_line> values;
  ...
  for (const auto &elt : ls->file_symtabs)
    {
      ...
      int index = 0;
      bool exact = false;
      val.symtab = find_line_symtab (elt, val.line, &index);

      /* New gdb logic doesn't return exact match bool, so we need to check
       * if the line number matches. */
      if (val.symtab && val.symtab->linetable())
        exact = val.symtab->linetable()->item[index].line == val.line;
#else
