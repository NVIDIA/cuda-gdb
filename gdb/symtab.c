  ...
  for (compunit_symtab *s : objfile->compunits ())
    {
      ...
      result.block = block;
      lookup_name_info lookup_name (name, symbol_name_match_type::FULL);
      result.symbol = block_lookup_symbol (block, lookup_name,
				 domain);
      if (result.symbol)
	{
...
#ifdef NVIDIA_CUDA_GDB
	  /* CUDA - interleaving */
	  /* CUDA objfiles are not allocated as a single block of memory.
	     Instead of erroneously getting the GLOBAL_BLOCK for those
	     objfiles, we find the innermost block containing the PC for that
	     symtab. */
...
symbol::get_maybe_copied_address () const
{
  ...
  if (this->is_objfile_owned () && this->objfile ()->cuda_objfile)
    minsym = lookup_minimal_symbol_linkage (this->objfile ()->pspace (), linkage_name, false, true);
  else
    minsym = lookup_minimal_symbol_linkage (this->objfile ()->pspace (), linkage_name, false);
#else
  bound_minimal_symbol minsym
...
minimal_symbol::get_maybe_copied_address (objfile *objf) const
{
  ...
  const char *linkage_name = this->linkage_name ();
#ifdef NVIDIA_CUDA_GDB
  bound_minimal_symbol found = lookup_minimal_symbol_linkage (objf->pspace (), linkage_name,
