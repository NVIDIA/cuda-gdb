...
add_partial_symbol (struct partial_die_info *pdi, struct dwarf2_cu *cu)
{
  ...
  if (where.has_value ())
      ...
      else
	{
	  ...
	  char *dmgl = symbol_find_demangled_name (&psymbol.ginfo, actual_name);
	  psymbol.ginfo.set_demangled_name (dmgl ? dmgl : actual_name,
...
make_segmented_type (cuda_sti_t &sti)
{
  ...
  copied_types = create_copied_types_hash (sti.objfile);
  marked_types = htab_create_alloc_ex (1, htab_hash_pointer, htab_eq_pointer,
                                       NULL, &sti.objfile->objfile_obstack,
                                       hashtab_obstack_allocate,
                                       dummy_obstack_deallocate);
  ...
  SYMBOL_TYPE (sti.sym) = copy_type_recursive (sti.objfile, SYMBOL_TYPE (sti.sym), copied_types);
  /* Then we mark the segment for each field of the type, marking the types that
     have already been visited using marked_types. */
  SYMBOL_TYPE (sti.sym) = make_segmented_type_aux (SYMBOL_TYPE (sti.sym), sti.flags, marked_types);
  /* Cleanup */
...
read_subroutine_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  switch (TYPE_CALLING_CONVENTION (ftype))
	...
	/* Set this subroutine as the "main" subroutine
	   for the program. */
	if (ftype->name () != NULL)
	  set_objfile_main_name (objfile, ftype->name (), cu->language);
	break;
      case DW_CC_normal:
      case DW_CC_nocall:
	default:
...
dwarf_record_line_1 (struct gdbarch *gdbarch, struct subfile *subfile, 
		     ...
		     struct dwarf2_cu *cu, 
		     const char *inline_function, ULONGEST context) 
{
  ...
  if (cu != nullptr) 
    cu->get_builder ()->record_line (subfile, line, addr, is_stmt, inline_function, context); 
}
#else
static void
dwarf_record_line_1 (struct gdbarch *gdbarch, struct subfile *subfile,
...
#ifdef NVIDIA_CUDA_GDB
  dwarf_record_line_1 (gdbarch, subfile, 0, address, true, cu, inline_function_name, context);
#else
	  ...
	  if (!end_sequence && !ignore_this_line)
				       ...
				       m_last_subfile))
		{
		  ...
		  dwarf_record_line_1 (m_gdbarch,
				       ...
				       m_currently_recording_lines ? m_cu : nullptr,
				       inline_function_name, context_line);
#else
		  dwarf_record_line_1 (m_gdbarch,
				       builder->get_current_subfile (),
				       m_line, m_address, is_stmt,
	...
	return build_error_marker_type (cu, die);
	 ...
	 ours.  */
      ...
      if (cu->producer && strncmp (cu->producer, "PGCC", 4) == 0)
        {
          struct attribute *next_type = dwarf2_attr (type_die, DW_AT_type, cu);
          if (next_type) 
            {
		  ...
		  const char *filename)
{
  ...
  for (auto &cuda_symbol : cuda_symbols)
	...
	selected_language = language_cplus;
      ...
      sym->language_specific.demangled_name = symbol_find_demangled_name (msym, sym->linkage_name ());
      symbol_set_symtab (sym, COMPUNIT_FILETABS (symtab)); /* dummy */
...
cuda_decode_line_table (struct objfile *objfile)
{
  ...
  if (!dwarf2_per_objfile->per_bfd->line.s.section)
    return;
  dwarf2_per_cu_data *per_cu = create_cu_from_index_list (dwarf2_per_objfile->per_bfd,
  ...
  if (!per_cu)
    return;
  dwarf2_cu *cu = new dwarf2_cu (per_cu, dwarf2_per_objfile);
  if (!cu)
    return;
  cu->ancestor = nullptr;
  ...
  struct compunit_symtab *symtab = cu->start_symtab (objfile->original_name, NULL, 0);
  cuda_add_minsyms (cu, objfile, symtab, objfile->original_name);
  do
    {
	...
	break;
      ...
      dwarf_decode_lines (cu->line_header, NULL, cu, NULL, 0, 1);
