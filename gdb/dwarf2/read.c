  ...
  if (sti.sym->type ()->is_objfile_owned ())
    {
      ...
      sti.sym->set_type (
	  copy_type_recursive (sti.sym->type (), copied_types));
    }
  else
...
#ifdef NVIDIA_BUGFIX
      /* Compiler started emitting a new TAG for OptiX code which conflicts with Fortran DWARF handling.
         Skip these incorrect DIEs as there are duplicates anyway with a subroutine tag.
      */
      if (abbrev->tag == DW_TAG_entry_point && !parent_entry)
        {
          warning (_(DWARF_ERROR_PREFIX "DW_TAG_entry_point without a parent"));
          continue;
        }
#endif
  ...
  if (dwarf_line_debug)
    {
      gdb_printf (gdb_stdlog,
		  ...
		  " epilogue_begin %u, discrim %u context %lu %s\n",
		  m_line, m_file,
  ...
  if (sti.flags == TYPE_INSTANCE_FLAG_CUDA_GLOBAL && is_variable)
    {
	  ...
	  = lookup_minimal_symbol (current_program_space, name, objf);
...
#ifdef NVIDIA_CUDA_GDB
	{
	  /* For CUDA device code, DW_AT_name might be mangled but without
	     DW_AT_linkage_name. In this case, use name directly to match
	     the simplified behavior that replaced the cuda_old_name WAR.
	     For host code (especially C++ with namespaces), use physname
	     to enable qualified name lookups. */
	  if (die->tag == DW_TAG_subprogram
	      && cuda_is_bfd_cuda (objfile->obfd.get ()))
...
#ifdef NVIDIA_BUGFIX
  /* If we don't have a name, try to get it from the specification.
     This handles cases like variables defined outside their declaring 
     namespace/class with DW_AT_specification.
     
     We only do this for variables to avoid breaking OpenMP offloading
     where following specification for functions can produce semi-demangled
     names that break cuobjdump invocation. Functions without DW_AT_name
     will fall back to DW_AT_linkage_name below. */
  if (attr_name == nullptr 
      && (die->tag == DW_TAG_variable || die->tag == DW_TAG_constant))
    {
      struct dwarf2_cu *spec_cu = cu;
      struct die_info *spec_die = die_specification (die, &spec_cu);
	  ...
	  && dwarf2_attr (die, DW_AT_artificial, cu))
	{
	  ...
	  if (dwarf2_get_pc_bounds (die, &lowpc, &highpc, cu, nullptr, nullptr)
	  ...
	  else
	    {
		  ...
		  = lookup_minimal_symbol_by_pc (cu->per_objfile->relocate (lowpc));
	      if (bminsym.minsym)
		  ...
		  const char *filename)
{
  ...
  cu->start_compunit_symtab (objfile->original_name, NULL, low_pc);
  /* Create a filetable - get the compunit_symtab from the builder */
  struct compunit_symtab *cust = cu->get_builder ()->get_compunit_symtab ();
