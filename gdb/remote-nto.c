...
nto_add_thread (process_stratum_target *targ, int pid, int tid)
{
  ...
  return add_thread_with_info (targ, ptid_t (pid, tid, 0), private_thread_info_up (priv));
}
...
putpkt (const DScomm_t *const tran, const unsigned len)
{
  ...
  *p++ = FRAME_CHAR;
  ...
  serial_write (current_session->desc, (char *)buf2, p - buf2);
...
pdebug_target::pdebug_open (const char *name, int from_tty)
{
  ...
  if (baud_rate != -1)
    {
      serial_setbaudrate (current_session->desc, baud_rate);
    }
      ...
      enum gdb_signal sig)
{
  ...
  nto_trace (0) ("pdebug_resume(pid %d, tid %ld, step %d, sig %d)\n",
     ptid.pid(), ptid.lwp(),
     step, nto_gdb_signal_to_target (current_inferior ()->arch (), sig));
  ...
  tran.pkt.handlesig.sig_to_pass
    = nto_gdb_signal_to_target (current_inferior ()->arch (), sig);
  tran.pkt.handlesig.sig_to_pass =
  ...
  tran.pkt.kill.signo = nto_gdb_signal_to_target (current_inferior ()->arch (), sig);
  tran.pkt.kill.signo =
...
pdebug_target::nto_parse_notify (const DScomm_t * const recv, struct target_waitstatus *status)
{
  ...
  nto_trace(0) ("  nto_parse_notify end: pid=%d, tid=%d  ip=%s\n", pid,
		tid, paddress (current_inferior ()->arch (), stopped_pc));
...
fetch_regs (struct regcache *regcache, int regset, int supply)
{
  ...
  if (gdbarch_bfd_arch_info (current_inferior ()->arch ()) != NULL
      && strcmp (gdbarch_bfd_arch_info (current_inferior ()->arch ())->arch_name, "i386")
   == 0
     ...
     ULONGEST *const xfered_len)
{
  ...
  nto_trace (0) ("nto_write_bytes(to %s, from %p, len %d)\n",
     paddress (current_inferior ()->arch (), memaddr), myaddr, len);
      ...
      const ULONGEST len, ULONGEST *const xfered_len)
{
  const unsigned arch_len
    = gdbarch_bfd_arch_info (current_inferior ()->arch ())->bits_per_word;
...
#ifdef NVIDIA_CUDA_GDB
  /* If no arguments were provided, nto_parse_redirection returns NULL
     and does not initialize in/out/err. Ensure deterministic defaults
     and provide an empty argv so downstream code does not dereference
...
pdebug_target::nto_insert_breakpoint (CORE_ADDR addr, gdb_byte *contents_cache)
{
  ...
  nto_trace (0) ("nto_insert_breakpoint(addr %s, contents_cache %p) pid:%d\n",
                 paddress (current_inferior ()->arch (), addr), contents_cache, inferior_ptid.pid());
...
nto_add_commands (void)
{
  ...
  //TODO: modernize this with `filename_maybe_quoted_completer`
       ...
       CORE_ADDR memaddr, ULONGEST size)
{
  ...
  nto_trace (0) ("%s (addr:%s, size:%llu)\n", __func__, paddress (current_inferior ()->arch (), memaddr), (long long unsigned)size);
...
nto_hw_watchpoint (CORE_ADDR addr, int len, enum target_hw_bp_type type)
{
  ...
  nto_trace (0) ("nto_hw_watchpoint(addr %s, len %x, type %x)\n",
     paddress (current_inferior ()->arch (), addr), len, type);
...
#ifdef NVIDIA_CUDA_GDB
  gdb_printf (file, _("Remote executable: \"%s\"\n"), inf_rdat->remote_exe.c_str ());
#else
...
#ifdef NVIDIA_CUDA_GDB
  gdb_printf (file, _("Remote cwd: \"%s\"\n"), inf_rdat->remote_cwd.c_str ());
#else
