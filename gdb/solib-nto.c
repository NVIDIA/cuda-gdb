...
swap_link_map (const gdb_byte *const lm)
{
  ...
  struct type *ptr_type = builtin_type (current_inferior ()->arch ())->builtin_data_ptr;
...
nto_build_arch_path (void)
{
  ...
  if (strcmp (gdbarch_bfd_arch_info (current_inferior ()->arch ())->arch_name, "i386") == 0)
    {
      if (IS_64BIT())
  arch = "x86_64";
      else
  arch = "x86";
    ...
    {
      arch = gdbarch_bfd_arch_info (current_inferior ()->arch ())->arch_name;
 ...
 */
//TODO: This could be heavily optimized since the only caller is building an svr4 solib list and only needs linkmap info and the name
...
nto_solist_from_qnx_linkmap_note (void)
{
  ...
  qnt_link_map_sect = bfd_get_section_by_name (current_program_space->core_bfd(),
  ...
  bfd_get_section_contents (current_program_space->exec_bfd(), (asection *)qnt_link_map_sect, qlmp, 0,
      ...
      if (lm.l_name < header.strtabsz)
        {
          ...
          solib_up new_elem = std::make_unique<solib>();
          li->lm_addr = 0;
          li->l_addr_p = true; /* Do not calculate l_addr. */
          li->l_addr = lm.l_addr;
          /* On QNX we always set l_addr to image base address. */
          li->l_addr_inferior = lm.l_addr;
          li->l_ld = lm.l_ld;
          new_elem->lm_info = std::move(li);
          ...
          if (strcmp (soname, "PIE") == 0)
            {
              ...
              new_elem->so_name = path;
            }
          else
            {
              /* standard shared object */
              new_elem->so_name = soname;
            }
          compressedpath = path[strlen(path)-1] == '/';
          new_elem->so_original_name = std::string(path) + (compressedpath ? soname : "");
          ...
          sos.push_back(std::move(new_elem));
        }
    }
    ...
    CORE_ADDR target_addr)
{
  ...
  if ( ( bfd_seek(abfd, host_addr, SEEK_SET) != 0 )
      || ( bfd_read ((gdb_byte*)&host_word, sizeof (host_word), abfd)
          != sizeof (host_word) ) )
    return -1;
 ...
 */
int
nto_so_validate (const solib &so)
{
  gdb_byte *build_id;
  size_t build_idsz;
  ...
  /* is the BFD sane? */
  if (!bfd_check_format (so.abfd.get(), bfd_object))
    {
      warning("BFD is no BFD object?!");
      return 1;
    }

  if (bfd_get_flavour (so.abfd.get()) != bfd_target_elf_flavour)
    {
      warning("No ELF BFD!");
      return 1;
    }

  if (so.abfd.get()->build_id == NULL)
    {
      warning("BFD could not read build ID for %s!", so.so_name.c_str());
      return 1;
    }
  ...
  if (build_id == NULL)
    {
      ...
      const asection *const asec
          = bfd_get_section_by_name (so.abfd.get(), NOTE_GNU_BUILD_ID_NAME);
...
//          struct cleanup *cleanups = make_cleanup (xfree, note);
          ...
          if (target_read_memory (bfd_section_vma (asec)
              ...
              note_raw, bfd_sect_size) == 0)
            {
              ...
              if (build_idsz == so.abfd.get()->build_id->size)
                {
                  const char gnu[] = "GNU";
...
//          do_cleanups (cleanups);
  ...
  if (build_id != NULL)
    {
      int match=2;
      if( ( so.abfd.get()->build_id->size == build_idsz )
        && ( memcmp (build_id, so.abfd.get()->build_id->data,
         so.abfd.get()->build_id->size) == 0 ) ) {
        match=0;
	}
      else
	{
	  ...
	  warning ("Host file %s does not match target file %s",
		   so.so_name.c_str(), so.so_original_name.c_str() );
	}
  ...
  warning (_("Shared object \"%s\" could not be validated "
       "and will be ignored."), so.so_name.c_str());
...
#ifdef NVIDIA_BUGFIX
      /* CUDA - Silence warning */
      scoped_fd r = gdb_open_cloexec (solib, o_flags, 0);
      if (r.get () >= 0)
	{
...
struct solib_ops nto_svr4_so_ops;
