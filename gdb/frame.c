  ...
  while (!buffer.empty ())
      ...
      else
	{
	  ...
	  *unavailablep = !value->entirely_available ();
	  ...
	  copy (value->contents_all ().slice (value->offset () + offset, curr_len),
#else
  ...
  while (!buffer.empty ())
...
#ifdef NVIDIA_CHERRY_PICK
      struct value *value
	= frame_unwind_register_value (frame_info_ptr (next_frame->next),
				       regnum);
      ...
      if (value != nullptr && value->lval () == lval_computed)
	{
	  ...
	  copy (buffer.slice (0, curr_len),
		from_value->contents_raw ().slice (0, curr_len));
