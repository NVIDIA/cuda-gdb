...
//  const char *inferior_io_terminal = get_inferior_io_terminal ();
  ...
  /* Preserve executable for fallback in case redirection parsing returns NULL. */
  const char *exec_argv0 = argv[0];

  argv = nto_parse_redirection (argv, &in, &out, &err);

  /* If parsing produced no argv, ensure deterministic defaults and a minimal argv. */
  if (argv == NULL)
    {
      in = "";
      out = "";
      err = "";
      argv = XCNEWVEC (const char *, 2);
      argv[0] = exec_argv0;
      argv[1] = NULL;
    }
#endif
