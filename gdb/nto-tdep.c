  ...
  if (inf_data->bind_func_p == 0)
    {
      ...
      struct bound_minimal_symbol resolver
          = lookup_minimal_symbol (current_program_space, "__resolve_func");

      if (resolver.minsym && resolver.objfile)
        {
	  struct bound_minimal_symbol bind_f
              = lookup_minimal_symbol (current_program_space, "__bind_func", resolver.objfile);
...
nto_get_siginfo_from_procfs_status (const void *const ps, void *siginfo)
{
  ...
  struct type *ptr_t = builtin_type (current_inferior ()->arch ())->builtin_data_ptr;
  ...
  if (dst->si_code <= 0) // SI_FROMUSER
    {
      ...
      == gdbarch_gdb_signal_to_target(current_inferior ()->arch (), GDB_SIGNAL_CHLD))
    {
      /* child termination */
      dst->nto_si_pid = extract_signed_integer((gdb_byte *) &src->nto_si_pid,
	  sizeof(src->nto_si_pid), nto_byte_order);
...
static void
nto_solib_loaded_listener (const solib &solib)
{
  ...
  nto_trace (0) ("nto_solib_loaded_listener(%s)\n", solib.so_name.empty() ? "<unnamed>" : solib.so_name.c_str());

  switch (gdbarch_bfd_arch_info (current_inferior ()->arch ())->bits_per_word)
    {
    case 64:
      offsetof_e_phoff = 32;
      sizeof_e_phoff = 8;
  ...
  while (1)
    {
      gdb::byte_vector phdr_buf(sizeof_Elf_Phdr);
      ...
      if (target_read_memory (mem_phdr_addr, phdr_buf.data(), sizeof_Elf_Phdr))
        {
          nto_trace (0) ("Could not read phdr at 0x%lx\n", mem_phdr_addr);
          return;
        }

      p_type = extract_unsigned_integer (&phdr_buf.data()[offsetof_p_type],
           sizeof_p_type, nto_byte_order);
      if (p_type == PT_LOAD)
        {
          p_filesz = extract_unsigned_integer (&phdr_buf.data()[offsetof_p_filesz],
                sizeof_p_filesz, nto_byte_order);
          p_memsz = extract_unsigned_integer (&phdr_buf.data()[offsetof_p_memsz],
                sizeof_p_memsz, nto_byte_order);
          p_flags = extract_unsigned_integer (&phdr_buf.data()[offsetof_p_flags],
                sizeof_p_flags, nto_byte_order);
          p_align = extract_unsigned_integer (&phdr_buf.data()[offsetof_p_align],
                sizeof_p_align, nto_byte_order);

          if (solib.symbols_loaded)
            {
              file_phdr = find_load_phdr_2 (solib.abfd.get(),
              ...
              if (file_phdr == NULL)
                {
                  ...
                  warning ("Host file %s does not match target file %s.",
                      solib.so_name.c_str(), solib.so_original_name.c_str());
                  break;
                }
            }
        }
...
nto_new_thread_listener( thread_info *ti ) {
  ...
  if (!current_program_space->core_bfd()) return;
  ...
  asection *tsect=bfd_get_section_by_name( current_program_space->core_bfd(), sectname );
  ...
  if (bfd_seek ( current_program_space->core_bfd(), tsect->filepos, SEEK_SET) != 0) {
    warning ( "Cannot read %s from core!", sectname);
    return;
  }

  if( bfd_read ((gdb_byte *)&ps, sectsize, current_program_space->core_bfd() ) != sectsize ) {
...
nto_inferior_appeared_listener( inferior *inf ) {
  ...
  std::optional<gdb::byte_vector>& auxv_data = qnx_get_auxv_data (inf);
#else
  ...
  gdb::byte_vector tempbuf(sizeof_tempbuf);
  ...
  len = bfd_read ((gdb_byte *)&pinfo, sectsize, current_program_space->core_bfd());
  if (len != sectsize) {
  ...
  len = nto_read_auxv_from_initial_stack
    (initial_stack, tempbuf.data(), sizeof_tempbuf, IS_64BIT()? 16 : 8);
  gdb::byte_vector databuf(len);
  memcpy( databuf.data(), tempbuf.data(), len );
