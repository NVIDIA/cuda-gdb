...
/* cuda_frame_base_address is not ABI-dependent, since it only
   ...
   compilations. */
static CORE_ADDR
cuda_frame_base_address (const frame_info_ptr &this_frame, void **this_cache)
{
  struct cuda_frame_cache *cache;
  CORE_ADDR base;
...
cuda_frame_unwind_stop_reason (const frame_info_ptr &this_frame, void **this_cache)
{
  /*NS: TODO*/
...
static void
cuda_frame_this_id (const frame_info_ptr &this_frame, void **this_cache,
		    struct frame_id *this_id)
{
...
cuda_abi_frame_prev_pc (frame_info_ptr next_frame)
{
  ...
  if (level == 0)
    ...
    pc = cuda_state::lane_get_return_address (c.dev (), c.sm (), c.wp (),
					      c.ln (), level - 1);
  else
    pc = 0;
...
static struct value *
cuda_noabi_frame_prev_register (frame_info_ptr next_frame, void **this_cache,
				int regnum)
{
  struct gdbarch *gdbarch = get_frame_arch (next_frame);
  uint32_t pc_regnum = gdbarch_pc_regnum (gdbarch);
...
static struct value *
cuda_frame_prev_register (const frame_info_ptr &next_frame, void **this_cache,
			  int regnum)
{
...
cuda_frame_base_sniffer (const frame_info_ptr &next_frame)
{
...
cuda_frame_sniffer (frame_info_ptr next_frame)
{
  ...
  if (frame_debug)
    gdb_printf (gdb_stdlog, "{ cuda_frame_sniffer (frame=%d) -> %d }\n",
		next_level, !!unwind);
...
CORE_ADDR
cuda_unwind_pc (struct gdbarch *gdbarch, const frame_info_ptr &next_frame)
{
  CORE_ADDR pc;
