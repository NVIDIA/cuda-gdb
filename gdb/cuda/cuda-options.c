 ...
 * set cuda memcheck
 */
static bool cuda_memcheck_auto = true;
static enum auto_boolean cuda_memcheck = AUTO_BOOLEAN_AUTO;
...
cuda_options_memcheck (void)
{
  ...
  const char *cuda_memcheck_env = env.get ("CUDA_MEMCHECK");
  cuda_memcheck_auto = (cuda_memcheck_env && strlen (cuda_memcheck_env) > 0 
	  && strcmp (cuda_memcheck_env,"1") == 0);
