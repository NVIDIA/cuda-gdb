...
public:
  ...
  cuda_instruction (const std::string &prefix, const std::string &opcode,
		    ...
		    std::optional<bool> is_control_flow = std::nullopt,
		    std::optional<bool> is_subroutine_call = std::nullopt,
		    std::optional<bool> is_barrier = std::nullopt)
...
private:
  ...
  /* populate the cache */
  std::optional<cuda_instruction> populate_from_elf_image (uint64_t pc);
  std::optional<cuda_instruction> populate_from_device_memory (uint64_t pc);
