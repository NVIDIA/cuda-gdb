...
static CUDBGResult
cudbgResumeWarpsUntilPC60 (uint32_t devId, uint32_t sm, uint64_t warpMask,
			   uint64_t virtPC)
{
  char *ipc_buf;
  CUDBGResult result;

  CUDBG_IPC_BEGIN (CUDBGAPIREQ_resumeWarpsUntilPC60);
  CUDBG_IPC_APPEND (&devId, sizeof (devId));
  CUDBG_IPC_APPEND (&sm, sizeof (sm));
...
static CUDBGResult
cudbgReadRegisterRange60 (uint32_t devId, uint32_t sm, uint32_t wp, uint32_t ln,
			  ...
			  uint32_t *registers)
{
  ...
  CUDBG_IPC_BEGIN (CUDBGAPIREQ_readRegisterRange60);
  CUDBG_IPC_APPEND (&devId, sizeof (devId));
  CUDBG_IPC_APPEND (&sm, sizeof (sm));
  CUDBG_IPC_APPEND (&wp, sizeof (wp));
  CUDBG_IPC_APPEND (&ln, sizeof (ln));
...
cudbgReadRegisterRange (uint32_t devId, uint32_t sm, uint32_t wp, uint32_t ln,
			uint32_t index, uint32_t numRegisters,
			uint32_t *registers, uint32_t *numRegistersRead)
{
  char *ipc_buf;
  CUDBGResult result;
  uint32_t num_read = 0;
  ...
  CUDBG_IPC_APPEND (&ln, sizeof (ln));
  CUDBG_IPC_APPEND (&index, sizeof (index));
  CUDBG_IPC_APPEND (&numRegisters, sizeof (numRegisters));

  CUDBG_IPC_REQUEST ((void **)&ipc_buf);
  CUDBG_IPC_RECEIVE (&result, &ipc_buf);
  CUDBG_IPC_RECEIVE (&num_read, &ipc_buf);
  CUDBG_IPC_RECEIVE_ARRAY (registers, num_read, &ipc_buf);

  if (numRegistersRead)
    *numRegistersRead = num_read;

  return result;
}

static CUDBGResult
cudbgInsertBreakpoint (uint32_t dev, uint64_t addr,
		       CUDBGBreakpointHandle *handle)
{
  char *ipc_buf;
  CUDBGResult result;

  CUDBG_IPC_BEGIN (CUDBGAPIREQ_insertBreakpoint);
  CUDBG_IPC_APPEND (&dev, sizeof (dev));
  CUDBG_IPC_APPEND (&addr, sizeof (addr));

  CUDBG_IPC_REQUEST ((void **)&ipc_buf);
  CUDBG_IPC_RECEIVE (&result, &ipc_buf);
  CUDBG_IPC_RECEIVE (handle, &ipc_buf);

  return result;
}

static CUDBGResult
cudbgRemoveBreakpoint (CUDBGBreakpointHandle handle)
{
  char *ipc_buf;
  CUDBGResult result;

  CUDBG_IPC_BEGIN (CUDBGAPIREQ_removeBreakpoint);
  CUDBG_IPC_APPEND (&handle, sizeof (handle));
...
static CUDBGResult
cudbgEnableBreakpoint (CUDBGBreakpointHandle handle)
{
  char *ipc_buf;
  CUDBGResult result;

  CUDBG_IPC_BEGIN (CUDBGAPIREQ_enableBreakpoint);
  CUDBG_IPC_APPEND (&handle, sizeof (handle));
...
static CUDBGResult
cudbgDisableBreakpoint (CUDBGBreakpointHandle handle)
{
  char *ipc_buf;
  CUDBGResult result;

  CUDBG_IPC_BEGIN (CUDBGAPIREQ_disableBreakpoint);
  CUDBG_IPC_APPEND (&handle, sizeof (handle));
...
static CUDBGResult
cudbgIsBreakpointEnabled (CUDBGBreakpointHandle handle, uint32_t *enabled)
{
  char *ipc_buf;
  CUDBGResult result;

  CUDBG_IPC_BEGIN (CUDBGAPIREQ_isBreakpointEnabled);
  CUDBG_IPC_APPEND (&handle, sizeof (handle));

  CUDBG_IPC_REQUEST ((void **)&ipc_buf);
  CUDBG_IPC_RECEIVE (&result, &ipc_buf);
  CUDBG_IPC_RECEIVE (enabled, &ipc_buf);

  return result;
}

static CUDBGResult
cudbgGetWarpHitBreakpoint (uint32_t dev, uint32_t sm, uint32_t wp,
			   CUDBGBreakpointHandle *handle)
{
  char *ipc_buf;
  CUDBGResult result;

  CUDBG_IPC_BEGIN (CUDBGAPIREQ_getWarpHitBreakpoint);
  CUDBG_IPC_APPEND (&dev, sizeof (dev));
  CUDBG_IPC_APPEND (&sm, sizeof (sm));
  CUDBG_IPC_APPEND (&wp, sizeof (wp));
...
static CUDBGResult
cudbgResumeWarpsUntilPC (uint32_t dev, uint32_t sm, uint64_t warpMask,
...
static CUDBGResult
cudbgSuspendAllDevices (uint32_t nonBlocking)
{
  char *ipc_buf;
  CUDBGResult result;

  CUDBG_IPC_BEGIN (CUDBGAPIREQ_suspendAllDevices);
  CUDBG_IPC_APPEND (&nonBlocking, sizeof (nonBlocking));
...
cudbgResumeAllDevices ()
{
  ...
  CUDBG_IPC_BEGIN (CUDBGAPIREQ_resumeAllDevices);
...
DEFINE_CUDBGAPI (
    ...
    cudbgGetElfImageByHandle, cudbgResumeWarpsUntilPC60,
    STUB_cudbgReadWarpState60, cudbgReadRegisterRange60, cudbgReadGenericMemory,
