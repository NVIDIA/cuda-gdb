...
cuda_breakpoint_hit_p (cuda_coords &coords)
{
  ...
  std::optional<cuda_coords> origin;
...
cuda_check_dwarf2_reg_ptx_virtual_register (uint64_t dwarf2_reg)
{
  ...
  auto it = cuda_ptx_virtual_reg_to_tagged_offset.find (dwarf2_reg);
  if (it != cuda_ptx_virtual_reg_to_tagged_offset.end ())
    return it->second;
...
cuda_dwarf2_reg_to_regnum (struct gdbarch *gdbarch, int reg)
{
  ...
  /* Check if this is a PTX virtual register encoded as ASCII string */
  uint64_t checked_reg = cuda_check_dwarf2_reg_ptx_virtual_register (reg);
  if (checked_reg != reg)
    {
      /* It was a PTX virtual register, use the tagged offset */
      reg = checked_reg;
    }

  /* Check if this is an ASCII encoded regular register (starting with 'R') */
  checked_reg = cuda_check_dwarf2_reg_ascii_encoded_register (gdbarch, reg);
  if (checked_reg != reg)
    {
      /* It was an ASCII encoded register, use the decoded value */
      reg = checked_reg;
      /* Try again with the decoded register */
      regno = cuda_decode_if_recognized (gdbarch, reg);
      if (regno != -1)
	return regno;
    }
...
cuda_pseudo_register_write (struct gdbarch *gdbarch,
			    const frame_info_ptr &frame, int regnum,
			    gdb::array_view<const gdb_byte> buf)
{
  ...
  if (cuda_special_regnum_p (gdbarch, regnum))
    {
      const auto regmap = regmap_get_search_result ();
      cuda_special_register_write (regmap, buf.data ());
      return;
    }
  error (_ ("%s: Invalid GDB register %d"), __FUNCTION__, regnum);
}
...
cuda_get_symbol_address (const char *name)
{
  struct bound_minimal_symbol msym
      = lookup_minimal_symbol (current_program_space, name, nullptr);
...
{
  /* If this is a CUDA built-in symbol address, just return default address
   * class.  */
  if (address >= CUDBG_BUILTINS_MAX)
    return 0;
...
cuda_segment_address_from_core_address (CORE_ADDR address)
{
  ...
  return address
	 & ~(cuda_address_class_mask | cuda_address_class_multiplex_mask);
}

const char *
cuda_address_class_to_name (ptxStorageKind address_class)
{
...
static int
cuda_type_instance_flags_to_address_class (type_instance_flags type_flags)
{
  switch (type_flags & TYPE_INSTANCE_FLAG_CUDA_ALL)
    {
    case TYPE_INSTANCE_FLAG_CUDA_CODE:
      return ptxCodeStorage;
    case TYPE_INSTANCE_FLAG_CUDA_CONST:
      return ptxConstStorage;
    case TYPE_INSTANCE_FLAG_CUDA_GENERIC:
      return ptxGenericStorage;
    case TYPE_INSTANCE_FLAG_CUDA_GLOBAL:
      return ptxGlobalStorage;
    case TYPE_INSTANCE_FLAG_CUDA_PARAM:
      return ptxParamStorage;
    case TYPE_INSTANCE_FLAG_CUDA_SHARED:
      return ptxSharedStorage;
    case TYPE_INSTANCE_FLAG_CUDA_LOCAL:
      return ptxLocalStorage;
    case TYPE_INSTANCE_FLAG_CUDA_REG:
      return ptxRegStorage;
    case TYPE_INSTANCE_FLAG_CUDA_UREG:
      return ptxURegStorage;
    case TYPE_INSTANCE_FLAG_CUDA_MANAGED:
      return ptxGlobalStorage;
    default:
      return ptxUNSPECIFIEDStorage;
    }
}
...
static int
cuda_read_memory_with_address_class_1 (CORE_ADDR address,
				       ptxStorageKind address_class,
				       gdb_byte *buf, int len,
				       uint64_t &hostaddr)
{
  gdb_assert (buf);

  // Set the fallback host address to the original address
  hostaddr = address;

  if (cuda_managed_address_p (address))
    {
      cuda_debugapi::read_global_memory (address, buf, len);
      return 0;
    }

  /* Check for non-focus global memory accesses */
  if (address_class == ptxGlobalStorage || address_class == ptxConstStorage)
    {
      cuda_debugapi::read_global_memory (address, buf, len);
      return 0;
    }

  /* If no device focus, return 1 so that the host path is tried next */
  if (!cuda_current_focus::isDevice ())
    return 1;
  ...
  switch (address_class)
    ...
    case ptxGlobalStorage:
      ...
      error (
	  "cuda_read_memory_with_address_class_1: unexpected address class %s",
	  cuda_address_class_to_name ((ptxStorageKind)address_class));
	...
	try
	  {
	    ...
	    // generic address that must be translated to a host
	    // address and accessed through the host. Other errors
	    // result in an exception being raised.
	    if (cuda_debugapi::read_generic_memory (
		    c.dev (), c.sm (), c.wp (), c.ln (), address, buf, len))
	      return 0;
	  }
	catch (const gdb_exception_error &e)
	  {
	    cuda_trace ("Exception from read_generic_memory() in "
			"cuda_read_memory_with_address_class_1(0x%lx, %s, %d)",
			(uint64_t)address,
			cuda_address_class_to_name (address_class), len);
	    throw;
	  }
	...
	// hostaddr. This call will throw an exception if the address
	// cannot be translated.
	uint64_t mapped_addr = 0;
	if (cuda_debugapi::get_host_addr_from_device_addr (c.dev (), address,
							   &mapped_addr))
    ...
    default:
      error (_ ("Unsupported address class for read: %d (0x%lx)"),
	     address_class, (uint64_t)address);
    }

  return 1;
}

// Return 0 if the read was successful, 1 if the read failed
int
cuda_read_memory_with_address_class (CORE_ADDR segmented_address,
				     int address_class, gdb_byte *buf, int len,
				     uint64_t &hostaddr)
{
  // Set the fallback host address to the segmented address
  hostaddr = segmented_address;

  // If CUDA isn't initialized return 1 so that the host path is tried next
  if (!cuda_debugging_enabled)
    return 1;

  const auto address_class_name
      = cuda_address_class_to_name ((ptxStorageKind)address_class);
  cuda_trace ("cuda_read_memory_with_address_class(0x%lx, %s, %d)",
	      (uint64_t)segmented_address, address_class_name, len);

  const auto result = cuda_read_memory_with_address_class_1 (
      segmented_address, (ptxStorageKind)address_class, buf, len, hostaddr);
  if (result == 0)
    {
      if ((len == 1) || (len == 2) || (len == 4) || (len == 8))
	{
	  const uint64_t value = (len == 1)   ? *(uint8_t *)buf
				 : (len == 2) ? *(uint16_t *)buf
				 : (len == 4) ? *(uint32_t *)buf
					      : *(uint64_t *)buf;
	  cuda_trace ("cuda_read_memory_with_address_class(0x%lx, %s, %d) "
    ...
    cuda_trace ("cuda_read_memory_with_address_class(0x%lx, %s, %d) "
		"failed, requesting fallback to host access at 0x%lx",
		(uint64_t)segmented_address, address_class_name, len,
		hostaddr);
  return result;
}
...
// Return 0 if the read was successful, 1 if the read failed
int
cuda_read_memory (gdbarch *gdbarch, CORE_ADDR address, int address_class,
		  gdb_byte *buf, int len)
{
  const auto address_class_name
      = cuda_address_class_to_name ((ptxStorageKind)address_class);
  ...
  if (cuda_debugging_enabled)
    {
      /* extract the segmented address from the core address */
      const auto segmented_address
	  = gdbarch_segment_address_from_core_address (gdbarch, address);
      try
	{
	  // If this is a CUDA address, read from the appropriate segment
	  result = cuda_read_memory_with_address_class (
	      segmented_address, address_class, buf, len, hostaddr);
	}
      catch (gdb_exception_error &e)
	{
	  cuda_trace (
	      "Exception in cuda_read_memory_with_address_class(0x%lx, "
	      "%s, %d): %s",
	      (uint64_t)segmented_address, address_class_name, len, e.what ());
	  throw;
	}
    }

  // If CUDA is not initialized or cuda_read_memory_with_address_class()
  // returned non-zero, try to read from the host.
  if (result != 0)
    {
      // Catch exceptions for logging purposes only, then rethrow.
      try
	{
	  // Either succeeds or throws an exception.
	  cuda_trace (
	      "cuda_read_memory(0x%lx, %s, %d): fallback to host at 0x%lx",
	      (uint64_t)address, address_class_name, len,
	      hostaddr ? hostaddr : address);
	  read_memory (hostaddr ? hostaddr : address, buf, len);
	  if ((len == 1) || (len == 2) || (len == 4) || (len == 8))
	    {
	      uint64_t value = (len == 1)   ? *(uint8_t *)buf
			       : (len == 2) ? *(uint16_t *)buf
			       : (len == 4) ? *(uint32_t *)buf
					    : *(uint64_t *)buf;
	      cuda_trace ("cuda_read_memory(0x%lx, %s, %d) = 0x%lx: fallback "
			  "to host at 0x%lx",
			  (uint64_t)address, address_class_name, len, value,
			  hostaddr ? hostaddr : address);
	    }
	  else
	    cuda_trace ("cuda_read_memory(0x%lx, %s, %d): fallback to host "
			"at 0x%lx",
			(uint64_t)address, address_class_name, len,
			hostaddr ? hostaddr : address);
	}
	...
	{
	  cuda_trace ("cuda_read_memory(0x%lx, %s, %d): Exception in "
		      "read_memory(0x%lx, %d): %s",
		      (uint64_t)address, address_class_name, len,
		      hostaddr ? hostaddr : (uint64_t)address, len, e.what ());
	  throw;
	}
    }
...
// Return 0 if the read was successful, 1 if the read failed
int
cuda_read_memory (gdbarch *gdbarch, CORE_ADDR address,
		  type_instance_flags flags, gdb_byte *buf, int len)
{
  /* We may reach here with gdbarch==x86/arm but with CUDA type instance flags
  set. Redirect to the CUDA gdbarch if detected.
  */
  if (flags & TYPE_INSTANCE_FLAG_CUDA_ALL)
    gdbarch = cuda_get_gdbarch ();

  const auto flags_address_class
      = gdbarch_type_instance_flags_to_address_class (gdbarch, flags);
  const auto ptr_address_class
      = gdbarch_address_class_from_core_address (gdbarch, address);
  const auto address_class
      = ptr_address_class == 0 ? flags_address_class : ptr_address_class;
  const auto segmented_address
      = gdbarch_segment_address_from_core_address (gdbarch, address);
  return cuda_read_memory (gdbarch, segmented_address, address_class, buf,
			   len);
}
		  ...
		  int len)
{
  ...
  // If val->arch() is CUDA gdbarch and val->stack() is true and the type has
  // no CUDA flags set, add the CUDA_LOCAL flag. This catches a case where a
  // local memory (stack) address is passed in without tag bits set and a type
  // without CUDA instance flags.
  auto flags = TYPE_CUDA_ALL (type);
  if ((val->arch () == cuda_get_gdbarch ()) && val->stack ()
      && !(flags & TYPE_INSTANCE_FLAG_CUDA_ALL))
    flags |= TYPE_INSTANCE_FLAG_CUDA_LOCAL;

  return cuda_read_memory (val->arch (), address, flags,
			   val->contents_all_raw ().data (), len);
}

// Write to CUDA memory taking into account the type instance flags
// and device focus. If the address is not a CUDA address, or it's a
// generic pointer to memory that has to be read from the host, then
// return 1.  Otherwise, if the memory can be read from the GPU, do so
// and return 0 on success or raise an exception on error.
int
cuda_write_memory_with_address_class (CORE_ADDR address, int address_class,
				      ...
				      uint64_t &hostaddr)
{
  ...
  if ((len == 1) || (len == 2) || (len == 4) || (len == 8))
    {
      const uint64_t value = (len == 1)	  ? *(uint8_t *)buf
			     ...
			     : (len == 4) ? *(uint32_t *)buf
					  : *(uint64_t *)buf;
      cuda_trace (
	  "cuda_write_memory_with_address_class(0x%lx, %s, %d) value 0x%lx",
	  (uint64_t)address,
	  cuda_address_class_to_name ((ptxStorageKind)address_class), len,
	  value);
    }
  else
    cuda_trace ("cuda_write_memory_with_address_class(0x%lx, %s, %d)",
		(uint64_t)address,
		cuda_address_class_to_name ((ptxStorageKind)address_class),
		len);

  // Global or managed memory accesses - does not require device focus
  if ((address_class == ptxGlobalStorage) || cuda_managed_address_p (address))
    {
      cuda_debugapi::write_global_memory (address, buf, len);
      return 0;
    }

  /* If focused on the host, indicate that the host path should be tried next
   */
  if (!cuda_current_focus::isDevice ())
    return 1;
  ...
  switch (address_class)
	...
	if (!cuda_current_active_elf_image_uses_abi ())
	  error ("Register write requires ABI support but current image "
		 "doesn't use ABI");
	...
	try
	  {
	    ...
	    // address and accessed through the host. Other errors
	    // result in an exception being raised.
	    if (cuda_debugapi::write_generic_memory (
		    c.dev (), c.sm (), c.wp (), c.ln (), address, buf, len))
	      return 0;
	  }
	catch (const gdb_exception_error &e)
	  {
	    cuda_trace (
		"cuda_write_memory_with_address_class(0x%lx, %s, %d): "
		"Exception from write_generic_memory(): %s",
		(uint64_t)address,
		cuda_address_class_to_name ((ptxStorageKind)address_class),
		len, e.what ());
	    throw;
    ...
    case ptxLocalStorage:
      {
	const auto result = cuda_debugapi::write_local_memory (
	    c.dev (), c.sm (), c.wp (), c.ln (), address, buf, len);
	return result ? 0 : 1;
      }
    ...
    default:
      error (_ ("Unknown address class for "
		"cuda_write_memory_with_address_class(0x%lx, %d, %d)"),
	     (uint64_t)address, (int)address_class, len);
    }
  /* Fall through to host access */
  return 1;
}
...
/* If there is an address class associated with this value, we've
   ...
   appropriate segment. Returns 0 on success, 1 on error.*/
int
cuda_write_memory (gdbarch *gdbarch, CORE_ADDR segmented_address,
		   int address_class, const gdb_byte *buf, int len)
{
  ...
  if (cuda_debugging_enabled)
    {
      try
	{
	  result = cuda_write_memory_with_address_class (
	      segmented_address, address_class, buf, len, hostaddr);
	}
      catch (const gdb_exception_error &e)
	{
	  cuda_trace (
	      "cuda_write_memory(0x%lx, %s, %d): Exception in "
	      "cuda_write_memory_with_address_class(): %s",
	      (uint64_t)segmented_address,
	      cuda_address_class_to_name ((ptxStorageKind)address_class), len,
	      e.what ());
	  throw;
	}
  ...
  if (result != 0)
      ...
      try
	{
	  // Either succeeds or throws an exception.
	  cuda_trace (
	      "cuda_write_memory(0x%lx, %s, %d): fallback to host at 0x%lx",
	      (uint64_t)segmented_address,
	      cuda_address_class_to_name ((ptxStorageKind)address_class), len,
	      hostaddr ? hostaddr : segmented_address);
	  write_memory (hostaddr ? hostaddr : segmented_address, buf, len);
	}
      catch (gdb_exception_error &e)
	{
	  cuda_trace (
	      "cuda_write_memory(0x%lx, %s, %d): Exception in "
	      "write_memory(0x%lx, %d): %s",
	      (uint64_t)segmented_address,
	      cuda_address_class_to_name ((ptxStorageKind)address_class), len,
	      (uint64_t)(hostaddr ? hostaddr : segmented_address), len,
	      e.what ());

	  // CUDA - if not managed memory, rethrow the exception
	  if (!cuda_managed_address_p (segmented_address))
	    throw;

	  cuda_debugapi::write_global_memory (segmented_address, buf, len);
	}
    }
  return 0;
}
...
/* If there is an address class associated with this value, we've
   ...
   appropriate segment. Returns 0 on success, 1 on error.*/
int
cuda_write_memory (gdbarch *gdbarch, CORE_ADDR address,
		   type_instance_flags flags, const gdb_byte *buf, int len)
{
  /* We may reach here with gdbarch==x86/arm but with CUDA type instance flags
  set. Redirect to the CUDA gdbarch instance flags conversion if so. Otherwise,
  use the default gdbarch.
  */
  if (flags & TYPE_INSTANCE_FLAG_CUDA_ALL)
    gdbarch = cuda_get_gdbarch ();
  ...
  const auto segmented_address
      = gdbarch_segment_address_from_core_address (gdbarch, address);
  return cuda_write_memory (gdbarch, segmented_address, address_class, buf,
			    len);
}
					 ...
					 uint64_t &end_pc, uint32_t &inst_size)
{
  ...
  // Not const as the disassembler will be updating it's cache of
  // instructions
  auto disassembler = module->disassembler ();
  gdb_assert (disassembler);
...
cuda_sstep_execute (ptid_t ptid)
{
  ...
  /* Compute predicates: check if the grid, sm, or warp changed */
  const bool has_valid_coord
      = cuda_sstep_info.grid_id_active && cuda_sstep_info.coord.valid ();
  const bool grid_id_changed
      = has_valid_coord
	&& (cuda_sstep_info.coord.logical ().gridId () != l.gridId ());
  const bool sm_changed
      = has_valid_coord
	&& ((cuda_sstep_info.coord.physical ().dev () != p.dev ())
	    || (cuda_sstep_info.coord.physical ().sm () != p.sm ()));
  ...
  /* This shouldn't happen unless single-stepping a CNP kernel */
  if (grid_id_changed)
    {
  ...
  /* If the sm changed, reset to current warp only */
  if (sm_changed)
    {
      /* Context changed or mask is stale; reset to current warp only. */
      cuda_api_clear_mask (&cuda_sstep_info.warp_mask);
    }

  /* Always set the current warp to ensure forward progress */
  cuda_api_set_bit (&cuda_sstep_info.warp_mask, p.wp (), 1);
...
static int
cuda_register_to_value (const frame_info_ptr &frame, int regnum,
...
static void
cuda_value_to_register (const frame_info_ptr &frame, int regnum,
			struct type *type, const gdb_byte *from)
{
  ...
  put_frame_register (frame, regnum,
		      gdb::make_array_view (from, type->length ()));
}
...
cuda_gdbarch_init (struct gdbarch_info info, struct gdbarch_list *arches)
{
  ...
  set_gdbarch_type_instance_flags_to_address_class (
      gdbarch, cuda_type_instance_flags_to_address_class);
...
{
  if (!cuda_options_preserve_tmp_data ())
    {
...
/* Determine if symbol refers to the __device_stub_ function corresponding to
  ...
  from the device stub mangled name and mark it as a shadow function. Can't
  use demangle_symbol here as the format appears non-standard.
...
cuda_new_objfile (objfile *objfile)
{
  ...
  for (auto msymbol : objfile->msymbols ())
				      ...
				      candidate_name))
	{
	  ...
	  auto shadow_minsym = lookup_minimal_symbol (
	      current_program_space, candidate_name.c_str (), objfile);
	  if (!shadow_minsym.minsym)
	    {
	      /* At this point we cannot trust the guessed linkage name. But
		 we can attempt to do a language demangle and lookup with
		 that. We always want to demangle using C++ as CUDA compiler
		 emits C++ mangled names for shadow functions.
	       */
	      ...
	      if (demangled_name)
		{
		  ...
		  /* Multiple minsyms can have the same demangled name. We
		     need to iterate over every matching candidate. */
...
cuda_free_objfile (objfile *objfile)
{
  ...
  for (auto msymbol : objfile->msymbols ())
    {
      /* We track the linkage name, so just always try to remove that from
	 the device stubs set. */
