  ...
  try
    {
      ...
      old_gdbarch = current_inferior ()->arch ();
      current_inferior ()->set_arch (cuda_get_gdbarch ());
      ...
      if (!cuda_current_focus::isDevice ())
	{
	  ...
	  if (coord.size ())
		...
		current_inferior ()->set_arch (old_gdbarch);
	    }
	}
...
{
  add_target (cuda_core_target_info, cuda_core_target_open, nullptr);
}
