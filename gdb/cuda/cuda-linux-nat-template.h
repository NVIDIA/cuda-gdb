					  ...
					  ULONGEST *xfered_len)
{
  *xfered_len = 0;
  ...
  if (!cuda_current_focus::isDevice ())
    {
      const auto status = BaseTarget::xfer_partial (
  ...
  uint64_t hostaddr = offset;
  switch (object)
    {
    case TARGET_OBJECT_MEMORY:
    case TARGET_OBJECT_STACK_MEMORY:
      {
	/* If focused on the device, all stack memory is local storage */
	if (cuda_current_focus::isDevice ())
	  {
	    const auto gdbarch = get_current_arch ();
	    auto address_class = ptxUNSPECIFIEDStorage;
	    if (object == TARGET_OBJECT_STACK_MEMORY)
	      address_class = ptxLocalStorage;
	    else if (object == TARGET_OBJECT_MEMORY)
	      {
		/* Extract the address class from the core address */
		address_class
		    = (ptxStorageKind)gdbarch_address_class_from_core_address (
			gdbarch, offset);
		/* If we cannot determine the address class, try generic
		 * storage before falling back to host access */
		if (address_class == ptxUNSPECIFIEDStorage)
		  address_class = ptxGenericStorage;
	      }
	    /* We have a CUDA address class, try to read/write from the
	     * appropriate segment */
	    const auto segment_address
		= gdbarch_segment_address_from_core_address (gdbarch, offset);
	    try
	      {
		if (readbuf ? !cuda_read_memory_with_address_class (
				  segment_address, address_class, readbuf, len,
				  hostaddr)
			    : !cuda_write_memory_with_address_class (
				  segment_address, address_class, writebuf,
				  len, hostaddr))
		  {
		    *xfered_len = len;
		    return TARGET_XFER_OK;
	      ...
	      {
		cuda_trace (_ ("Failed to read/write CUDA %s object at "
			       "address class "
			       "%s addr 0x%lx"),
			    (object == TARGET_OBJECT_STACK_MEMORY) ? "stack"
								   : "memory",
			    cuda_address_class_to_name (address_class),
			    (uint64_t)segment_address);
	      }
	    /* read/write failure. For stack memory with device focus, we stop
	       here and return an error. */
	    if (object == TARGET_OBJECT_STACK_MEMORY)
	      return TARGET_XFER_E_IO;
	  }
	/* Fall through to try host access */
	break;
      }
    ...
    default:
      cuda_trace (
	  _ ("Unsupported object for cuda_nat_linux::xfer_partial: %d"),
	  object);
      /* Fall through to try host access */
    }

  /* Try host accesses to the (possibly) remapped address (hostaddr) */
  return BaseTarget::xfer_partial (object, annex, readbuf, writebuf, hostaddr,
				   len, xfered_len);
}
...
{
  /* XXX potential race condition here. we kill the application, and will
     later kill the device when finalizing the API. Should split the process
     into smaller steps: kill the device, then kill the app, then kill the
...
/* This discusses how CUDA device exceptions are handled.  This
   ...
   3. cuda_wait ()
     ...
     been "signalled" (i.e. app has terminated) with a signal.  At this
   point, cuda_exception_is_valid (cuda_exception) is set back to false.
   Process mourning ensues and the world is a better place.
*/
...
cuda_nat_linux<BaseTarget>::resume (ptid_t ptid, int sstep, enum gdb_signal ts)
{
  ...
  /* In cuda-gdb we have two types of device exceptions :
     ...
     When a device side exception is hit, it sets cuda_exception in
     cuda_wait. In the case of a nonrecoverable exception, the cuda_resume
     call kills the host application and return early. The subsequent
     cuda_wait call cleans up the exception state. In the case of a
     recoverable exception, cuda-gdb must reset the exception state here and
     can then continue executing. In the case of CUDA_EXCEPTION_WARP_ASSERT,
     the handling of the exception (i.e. printing the assert message) is done
     as part of the cuda_wait call.
  */
  cuda_exception ex;
  if (ex.has_exception ())
    {
...
/*CUDA_WAIT:
  ...
  as unique signals (defined in signal.[c,h]).  Everything else is presented
				  ...
				  target_wait_flags target_options)
{
  ...
  if (cuda_sstep_is_active ())
      ...
      else
	{
	  ...
	  if (pending_sigint || check_quit_flag ())
	      ...
	      if (cuda_sstep_kernel_has_terminated ())
		{
		  ...
		  /* Only destroy the kernel that has been stepped to its
		   * exit
		   */
		  cuda_state::destroy_kernel (cuda_sstep_dev_id (),
					      cuda_sstep_grid_id ());

		  /* Consume any asynchronous events, if necessary.  We need
		   * to do this explicitly here, since we're taking the quick
		   * path out of this routine (and bypassing the normal check
		   * for API events). */
		  cuda_process_events (CUDA_EVENT_ASYNC);
  ...
  else
    {
      ...
      /* GDB reads events asynchronously without blocking. The target may
	 have taken too long to reply and GDB did not get any events back.
	 Check if this is the case and just return.  */
  ...
  /* Used to track if a suspend event was received. */
  bool devices_suspended = false;
  uint64_t broken_devices = 0;
  uint64_t faulted_devices = 0;
  auto handle_suspended_event = [&] (const CUDBGEvent &event) {
    switch (event.kind)
      {
      case CUDBG_EVENT_ALL_DEVICES_SUSPENDED:
	{
	  ...
	  /* This ensures that cuda_state recognizes that the devices have
	   * been suspended. */
	  for (auto dev = 0; dev < cuda_state::get_num_devices (); ++dev)
	    cuda_state::set_suspended_devices_mask (dev);
	  cuda_state::update_all_state (CUDBG_RESPONSE_TYPE_UPDATE);
  ...
  auto update_managed_memory = [&] () {
	...
	/* Mark inferior_ptid as not executing while reading object signal
	 * info*/
	...
	if (read == len && cuda_managed_address_p (addr))
	  {
	    if (ws->kind () == TARGET_WAITKIND_STOPPED)
	      ...
	      ws->set_signalled (
		  GDB_SIGNAL_CUDA_INVALID_MANAGED_MEMORY_ACCESS);
  ...
  /* Special case single stepping. */
  if (cuda_sstep_is_active ())
    {
      /* Update the info about the kernels */
      cuda_state::update_kernels_terminated ();
      /* Always check for an exception. We don't get suspend events while
  ...
  /* Handle the case where the devices have been suspended. */
  else if (devices_suspended)
    {
      /* Update the info about the kernels */
      cuda_state::update_kernels_terminated ();

      /* Handle exceptions. */
      // TODO: Check the exception based on the device mask
      if (faulted_devices)
	{
  ...
  /* Handle the case for everything else */
  else if (cuda_debugapi::get_attach_state () == CUDA_ATTACH_STATE_APP_READY)
    {
	 ...
	 Preferably switch focus to a device if possible */
      ...
      /* Always force suspend on initial attach. */
      force_suspend ();
  ...
  else if (cuda_notification_received ())
    {
	 ...
	 but we drained no events from either queue in this wait cycle. */
      cuda_trace ("cuda_wait: spurious CUDA notification, nothing drained");
	   ...
	   || ws->kind () == TARGET_WAITKIND_SIGNALLED)
    {
      /* We are stopped for a non-CUDA reason. When the host
	 stops, we need to ensure we halt the device. */
      ...
      if (exp.has_exception ())
	{
	  /* Found a CUDA exception - report it and skip all other checks.
	   * The exception signal takes priority over breakpoints, managed
      ...
      else
	{
	  cuda_coord_set<cuda_coord_set_type::lanes, select_valid | select_trap
							 | select_current_clock
							 | select_sngl>
	      broken_lanes{ cuda_coords::wild () };

	  if (broken_lanes.size ()
	      && cuda_breakpoint_hit_p (tp->new_cuda_coords))
	    {
	      cuda_trace (
					       ...
					       struct bp_target_info *bp_tgt)
{
  ...
  bool inserted = false;
  cuda_module *module
      = cuda_module::find_cuda_module_by_address (bp_tgt->reqstd_address);
  if (module)
    {
