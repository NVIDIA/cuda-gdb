...
template <class parent> class cuda_remote_target : public parent
{
public:
  ...
  void cuda_do_resume (ptid_t ptid, int sstep, int host_sstep,
		       enum gdb_signal ts);
...
void
cuda_remote_target<parent>::cuda_do_resume (ptid_t ptid, int sstep,
					    int host_sstep, enum gdb_signal ts)
{
  uint32_t dev;
...
cuda_initialize_remote_target (struct target_ops *ops)
{
  ...
  if (!symbols_are_set)
    {
      ...
      cuda_remote_set_symbols ((struct remote_target *)ops, true,
			       &symbols_are_set);
      if (!symbols_are_set)
	{
	  warning (
	  ...
	  /* If the above failed, assume it's incompatible and only send
	     the core symbols */
	  cuda_remote_set_symbols ((struct remote_target *)ops, false,
				   &symbols_are_set);
	  if (!symbols_are_set)
	    {
	      return false;
  ...
  cuda_remote_initialize (remote, &get_debugger_api_res, &set_callback_api_res,
...
template <class parent>
ptid_t
cuda_remote_target<parent>::wait (ptid_t ptid, struct target_waitstatus *ws,
				  target_wait_flags target_options)
{
  ...
  if (!cuda_options_device_resume_on_cpu_dynamic_function_call ())
	...
	return parent::wait (ptid, ws, target_options);
  ...
  if (cuda_sstep_is_active ())
	...
	{
	  /* Check if C-c was sent to a remote application or if quit_flag is
	   * set. quit_flag is set by gdb handle_sigint() signal handler */
#ifdef __QNXTARGET__
  ...
  /* Try to initialize CUDA if we haven't done so already. This may throw an
   * exception, in which case we want to ignore it. */
  if (!cuda_initialized)
    {
      try
	{
	  auto res = cuda_initialize_remote_target (
	      current_inferior ()->top_target ());
	  if (!res)
	    {
	      cuda_trace ("cuda_wait: cuda_initialize_remote_target() failed, "
			  "return pid %d",
			  r.pid ());
	      cuda_current_focus::invalidate ();
	      return r;
  ...
  if ((ws->kind () == TARGET_WAITKIND_STOPPED
       || ws->kind () == TARGET_WAITKIND_SIGNALLED)
      && (ws->sig () == GDB_SIGNAL_BUS || ws->sig () == GDB_SIGNAL_SEGV) && tp)
    {
      uint64_t addr = 0;
...
template <class parent>
void
cuda_remote_target<parent>::store_registers (struct regcache *regcache,
					     int regno)
{
  uint64_t val;
  struct gdbarch *gdbarch = regcache->arch ();
...
cuda_remote_target<parent>::xfer_partial (enum target_object object,
					  const char *annex, gdb_byte *readbuf,
					  ...
					  ULONGEST *xfered_len)
{
  ...
  if (!cuda_current_focus::isDevice ())
    {
      status = parent::xfer_partial (object, annex, readbuf, writebuf, offset,
				     len, xfered_len);
      return status;
    }
...
cuda_remote_attach (void)
{
  ...
  if (cuda_debugapi::get_attach_state () != CUDA_ATTACH_STATE_NOT_STARTED
    ...
    return;
#ifndef __QNXTARGET__
  if (!cuda_get_current_remote_target ()->remote_query_attached (
	  inferior_ptid.pid ()))
    return;
#endif
  ...
  do
      ...
      if (need_retry)
	{
	  ...
	  /* Our code server-side will forge an expected stop event, so that it
	   * will get reported to the client. */
...
cuda_remote_target<parent>::detach (inferior *inf, int from_tty)
{
  ...
  if (cuda_debugapi::get_attach_state () == CUDA_ATTACH_STATE_COMPLETE
      && cuda_get_current_remote_target ()->remote_query_attached (
	  inferior_ptid.pid ()))
