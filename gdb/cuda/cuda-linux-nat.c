...
{
  struct obstack *obstack = &(symtab->compunit ()->objfile ()->objfile_obstack);
  ...
  global_block->set_multidict (mdict_create_hashed_expandable (symtab->language ()));
  ...
  static_block->set_multidict (mdict_create_hashed_expandable (symtab->language ()));
...
cuda_create_builtins_objfile (void)
{
  ...
  objfile->compunit_symtabs
      = start_compunit_symtab (objfile, "<cuda-builtins>", NULL, 0, language_c);
