...
{
  std::optional<cuda_instruction> insn (cache_lookup (pc, source));
    ...
    const uint64_t pc, const disassembly_source source) const
{
  ...
  return std::optional<cuda_instruction> ();
}
...
cuda_module_disassembly_cache::populate_from_elf_image (const uint64_t pc)
{
  ...
  // If disassembly succeeded, look up the instruction in the cache
  if (disassembled_successfully)
...
cuda_module_disassembly_cache::parse_disasm_output_json (const int fd)
{
  ...
  parser.set_function_consumer ([this] (const schema_function &function) {
    ...
    for (const auto &insn : function.m_instructions)
      {
	/* When available, use JSON attributes provided by nvdisasm to identify
	   instruction characteristics (control-flow, subroutine-call,
	   inter-warp-barrier).
	   This avoids opcode string comparisons.
	   If attributes are not present (nullopt), the cuda_instruction class
	   will fall back to opcode string matching in eval_is_*() methods. */
	const bool insn_attrs_valid
	    = cuda_disasm_json::validate_sass_instruction_attributes (insn);
	CUDA_ERR_IF (!insn_attrs_valid, CUDA_TRACE_DISASSEMBLER,
		     "Invalid instruction attributes in JSON output for "
		     "instruction: %s",
		     insn.m_opcode.c_str ());

	instructions.emplace_back (
	    insn.m_predicate, insn.m_opcode, insn.m_operands, insn.m_extra,
	    insn.m_opt_is_control_flow, insn.m_opt_is_subroutine_call,
	    insn.m_opt_is_inter_warp_barrier);
      }
	  ...
	  if (!current_func.empty ())
	    {
		  ...
		  current_program_space, current_func.c_str (),
		  module->objfile ());
	      if (sym.minsym == NULL)
		{
		  cuda_trace_domain (CUDA_TRACE_DISASSEMBLER,
	...
	case disassembler_line_type::section_header:
	  if (!current_section.empty ())
	      ...
	      else
		{
	       ...
	       * minimal symbol */
	      struct bound_minimal_symbol sym = lookup_minimal_symbol (
		  current_program_space, sym_name.c_str (),
		  module->objfile ());
	      if (sym.minsym == NULL)
		{
		  cuda_trace_domain (CUDA_TRACE_DISASSEMBLER,
