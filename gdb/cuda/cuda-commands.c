...
void
bits_to_hex (std::stringstream &ss, const std::bitset<N> &val, size_t &pos,
             const int nbits)
{
  gdb_assert (nbits <= 8);
...
template <std::size_t N>
std::string
sms_bitset_to_hex (const uint32_t num_sms, const std::bitset<N> &val)
{
...
private:
  ...
  populate ()
  {
    ...
    cuda_iterator<T::iterator_type, T::iterator_mask, T::compare_type> coords{
      m_filter.coords ()
    };
...
public:
  /* Static members */
  constexpr static cuda_iterator_type iterator_type
      = cuda_iterator_type::devices;
  constexpr static underlying_type_t<cuda_iterator_mask_t> iterator_mask
      = select_all;
  constexpr static cuda_compare_type compare_type = cuda_compare_type::physical;
            ...
            coords.physical ().dev ()) }
  {
    ...
    cuda_state::device_get_active_sms_mask (m_device, m_active_sms_mask);
...
public:
  /* Static members */
  constexpr static cuda_iterator_type iterator_type = cuda_iterator_type::sms;
  constexpr static underlying_type_t<cuda_iterator_mask_t> iterator_mask
      = select_all;
  constexpr static cuda_compare_type compare_type = cuda_compare_type::physical;
  constexpr static command_t parser_command = CMD_FILTER;
  ...
  {
    // Ignore invalid sms
    if (!cuda_state::sm_is_valid (m_device, m_sm))
...
public:
        ...
        m_sm{ coords.physical ().sm () }, m_wp{ coords.physical ().wp () }
  {
    ...
    if (!cuda_state::warp_is_valid (m_device, m_sm, m_wp))
...
public:
  ...
  {
    if (!cuda_state::lane_is_valid (m_device, m_sm, m_wp, m_ln))
    ...
    auto active = cuda_state::lane_is_active (m_device, m_sm, m_wp, m_ln);
...
public:
  /* Static members */
  constexpr static cuda_iterator_type iterator_type
      = cuda_iterator_type::kernels;
  constexpr static underlying_type_t<cuda_iterator_mask_t> iterator_mask
      = select_all;
  constexpr static cuda_compare_type compare_type = cuda_compare_type::logical;
  constexpr static command_t parser_command = CMD_FILTER;
  static cuda_filters
  default_filter ()
  {
  ...
  explicit cuda_kernel (kernel_t kernel)
  {
    ...
    uint32_t num_sms = cuda_state::device_get_num_sms (m_device);
