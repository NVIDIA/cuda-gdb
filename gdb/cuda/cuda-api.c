...
void
cuda_debugapi::read_texture_memory (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t id, uint32_t dim, uint32_t *coords, void *buf, uint32_t sz)
{
  if (get_state () != CUDA_API_STATE_INITIALIZED)
    return;

  CUDBGResult res = m_cudbgAPI->readTextureMemory (dev, sm, wp, id, dim, coords, buf, sz);
  cuda_api_print_api_call_result (res);
...
{
  cuda_debugapi::instance ().read_texture_memory (dev, sm, wp, id, dim, coords, buf, sz);
}


void
cuda_debugapi::read_texture_memory_bindless (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t tex_symtab_index, uint32_t dim, uint32_t *coords, void *buf, uint32_t sz)
{
  if (get_state () != CUDA_API_STATE_INITIALIZED)
    return;

  CUDBGResult res = m_cudbgAPI->readTextureMemoryBindless (dev, sm, wp, tex_symtab_index, dim, coords, buf, sz);
...
{
  cuda_debugapi::instance ().read_texture_memory_bindless (dev, sm, wp, tex_symtab_index, dim, coords, buf, sz);
}
...
void
cuda_debugapi::memcheck_read_error_address (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln,
					    uint64_t *address, ptxStorageKind *storage)
{
  if (get_state () != CUDA_API_STATE_INITIALIZED)
    return;

  CUDBGResult res = m_cudbgAPI->memcheckReadErrorAddress (dev, sm, wp, ln, address, storage);
  cuda_api_print_api_call_result (res);
...
cuda_api_memcheck_read_error_address (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln,
                                      uint64_t *address, ptxStorageKind *storage)
{
  cuda_debugapi::instance ().memcheck_read_error_address (dev, sm, wp, ln, address, storage);
}
