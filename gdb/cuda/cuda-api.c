  ...
  if (res != CUDBG_SUCCESS)
    {
      ...
      get_error_string_ex (errStrEx, sizeof(errStrEx), nullptr);
      cuda_api_trace ("API call received result: %s, error message=%s",
		      cudbgGetErrorString((CUDBGResult) res), errStrEx);
    }
}
...
cuda_debugapi::get_cluster_dim (uint32_t dev, uint64_t gridId64, CuDim3 *cluster_dim)
{
  ...
  if (res != CUDBG_SUCCESS && res != CUDBG_ERROR_NOT_SUPPORTED)
    CUDA_API_ERROR_DEV (res, dev, "read the cluster dimensions");

  CUDA_API_TRACE_DEV (dev, "cluster_dim = (%u, %u, %u)",
...
cuda_debugapi::read_register_range (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t ln, uint32_t idx, uint32_t count, uint32_t *regs)
{
  ...
  for (int i = 0; i < count; ++i)
    CUDA_API_TRACE_DEV_SM_WARP_LANE (dev, sm, wp, ln, "R%u = %d (0x%x)", idx + i, regs[i], regs[i]);
}
...
cuda_debugapi::read_uregister_range (uint32_t dev, uint32_t sm, uint32_t wp, uint32_t idx, uint32_t count, uint32_t *regs)
{
  ...
  CUDA_API_TRACE_DEV_SM_WARP (dev, sm, wp, "");
  for (int i = 0; i < count; ++i)
    CUDA_API_TRACE ("UR%d = %d (0x%x)", idx + i, regs[i], regs[i]);
}
				 ...
				 uint32_t nsteps, cuda_api_warpmask *warp_mask)
{
  ...
  CUDBGResult res = m_instance.m_cudbgAPI->singleStepWarp (dev, sm, wp, nsteps, &warp_mask->mask);
  cuda_api_print_api_call_result (__FUNCTION__, res);
  ...
  CUDA_API_TRACE_DEV_SM_WARP (dev, sm, wp, "warp mask %" WARP_MASK_FORMAT,
                                         ...
                                         uint32_t numEntries)
{
  ...
  if (!done && (api_version ().m_revision >= 138))
    {
      // The newer version of the call is now the default
      name.append ("::getLoadedFunctionInfo");
      res = m_instance.m_cudbgAPI->getLoadedFunctionInfo (dev, handle, info, startIndex, numEntries);
      done = true;
    }
#endif
  if (!done && (api_version ().m_revision >= 131))
    {
      // Not yet implemented for where the starting index is not zero.
      gdb_assert (startIndex == 0);
  
#if CUDBG_API_VERSION_REVISION >= 138
      // Compiler with cudadebugger.h >= 138, but with a older client that
      // doesn't support the getLoadedFunctionInfo() interface which takes
      // the startIndex as a parameter
      name.append ("::getLoadedFunctionInfo_118");
      res = m_instance.m_cudbgAPI->getLoadedFunctionInfo118 (dev, handle, info, numEntries);
#else
      name.append ("::getLoadedFunctionInfo");
      res = m_instance.m_cudbgAPI->getLoadedFunctionInfo (dev, handle, info, numEntries);
#endif
      done = true;
    }
#endif

  // Do not call cuda_api_print_api_call_result() as it could result in
  // infinite recursion.

  if (!done)
    CUDA_API_ERROR_DEV (res, dev, "Failed to find getLoadedFunctionInfo() entrypoint to use");

  if (res != CUDBG_SUCCESS)
    CUDA_API_ERROR_DEV (res, dev, "%s failed: handle 0x%lx startIndex %u numEntries %u",
			name.c_str (), handle, startIndex, numEntries);

  CUDA_API_TRACE_DEV (dev, "%s handle 0x%lx startIndex %u numEntries %u",
		      name.c_str (), handle, startIndex, numEntries);
}
...
cuda_debugapi::get_loaded_function_info (uint32_t dev, uint64_t handle,
                                         ...
                                         uint32_t numEntries)
{
  get_loaded_function_info (dev, handle, info, 0, numEntries);
}
...
{
#if CUDBG_API_VERSION_REVISION >= 138
  if (api_state_initialized () && api_version ().m_revision >= 138)
    {
      CUDBGResult res = m_instance.m_cudbgAPI->getConstBankAddress (dev, sm, wp, bank, offset, address);
      cuda_api_print_api_call_result (__FUNCTION__, res);
