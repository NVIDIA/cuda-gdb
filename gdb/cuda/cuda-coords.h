...
/* NOTE: There's minimal support for clusters at the moment:
 *   - clusterIdx is a lazy field that's only filled for only particular use case
 *   - clusterIdx_p is true if clusterIdx has been lazily fetched
 *   - clusterIdx_valid is true if clusterIdx is valid (i.e. if the kernel uses clusters)
 */
typedef struct {
  bool valid;
  bool clusterIdx_p;
  bool clusterIdx_valid;
  uint64_t kernelId;
  uint64_t gridId;
  CuDim3 clusterIdx;
  CuDim3 blockIdx;
...
typedef enum {
  ...
  /* Special case for unexpected bpt.trap */
  CUDA_SELECT_TRAP  = 0x010,
  /* Select threads that match the current cuda_clock */
  CUDA_SELECT_CURRENT_CLOCK = 0x020,
} cuda_select_t;

#define CUDA_INVALID_COORDS_INIT				\
  { false, false, false, CUDA_INVALID, CUDA_INVALID,		\
...
#define CUDA_WILDCARD_COORDS_INIT				\
  { true, false, false, CUDA_WILDCARD, CUDA_WILDCARD,		\
...
void  cuda_coords_find_valid (cuda_coords_t wished, cuda_coords_t found[CK_MAX], cuda_select_t select_mask = CUDA_SELECT_VALID);
