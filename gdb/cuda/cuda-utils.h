...
/* Bypass non-fatal signals to the application during controlled application
 * resume Perform cleanup of the returned value to revert all changes in
 * debugger behaviour introduced by this routine
 */
void cuda_nat_bypass_signals_cleanup (unsigned char *ptr);
using cuda_gdb_bypass_signals_cleanup
    = FORWARD_SCOPE_EXIT (cuda_nat_bypass_signals_cleanup);
...
struct type;
struct symbol;
void cuda_ptx_cache_store_register (frame_info_ptr, int dwarf_regnum,
				    struct value *);
struct value *cuda_ptx_cache_get_register (frame_info_ptr, int dwarf_regnum,
					   struct type *);
void cuda_ptx_cache_refresh (void);
void cuda_ptx_cache_local_vars_iterator (const char *, struct symbol *,
					 frame_info_ptr);
...
static inline void
cuda_write_bool (CORE_ADDR addr, bool val)
{
