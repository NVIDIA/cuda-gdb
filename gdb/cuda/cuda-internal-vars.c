...
static inline uint32_t cuda_internal_var_laneid()
{
  return cuda_current_focus::get().physical().ln();
}

static inline uint32_t cuda_internal_var_warpid()
{
  return cuda_current_focus::get().physical().wp();
}

static inline uint32_t cuda_internal_var_nwarpid()
{
  return cuda_state::device_get_num_warps(cuda_current_focus::get().physical().dev());
}

static inline uint32_t cuda_internal_var_smid()
{
  return cuda_current_focus::get().physical().sm();
}

static inline uint32_t cuda_internal_var_nsmid()
{
  return cuda_state::device_get_num_sms(cuda_current_focus::get().physical().dev());
}

static inline uint32_t cuda_internal_var_devid()
{
  return cuda_current_focus::get().physical().dev();
}

static inline uint32_t cuda_internal_var_ndevid()
{
...
static inline CuDim3 cuda_internal_var_tid()
{
  return cuda_current_focus::get().logical().threadIdx();
}

static inline CuDim3 cuda_internal_var_ntid()
{
  return cuda_current_focus::get().logical().kernel()->block_dim();
}

static inline CuDim3 cuda_internal_var_ctaid()
{
  return cuda_current_focus::get().logical().blockIdx();
}

static inline CuDim3 cuda_internal_var_nctaid()
{
  return cuda_current_focus::get().logical().kernel()->grid_dim();
}

static inline uint64_t cuda_internal_var_gridid()
{
  return cuda_current_focus::get().logical().gridId();
}

static inline uint64_t cuda_internal_var_kernelid()
{
  return cuda_current_focus::get().logical().kernelId();
}

static inline uint32_t cuda_internal_var_nkernelid()
{
  cuda_coord_set<cuda_coord_set_type::kernels, select_valid> kernels_itr(cuda_coords::wild());
...
static inline CuDim3 cuda_internal_var_cluster_ctaid()
{
  check_cluster();
...
static inline CuDim3 cuda_internal_var_cluster_nctaid()
{
  check_cluster();
...
static inline uint32_t cuda_internal_var_cluster_ctarank()
{
  check_cluster();
...
static inline uint32_t cuda_internal_var_cluster_nctarank()
{
  check_cluster();
...
static inline CuDim3 cuda_internal_var_clusterid()
{
  check_cluster();
...
static inline CuDim3 cuda_internal_var_nclusterid()
{
  check_cluster();

  const CuDim3& grid_dim = cuda_current_focus::get().logical().kernel()->grid_dim();
  const CuDim3& cluster_dim = cuda_current_focus::get().logical().clusterDim();
  return {grid_dim.x / cluster_dim.x, grid_dim.y / cluster_dim.y, grid_dim.z / cluster_dim.z};
}

static inline bool cuda_internal_var_is_explicit_cluster()
{
...
/* Masks */
static inline uint32_t cuda_internal_var_lanemask_eq()
{
  uint32_t mask = 1;
...
static inline uint32_t cuda_internal_var_lanemask_le()
{
...
static inline uint32_t cuda_internal_var_lanemask_lt()
{
  uint32_t mask = cuda_internal_var_lanemask_eq();
  mask -= 1;
  return mask;
...
static inline uint32_t cuda_internal_var_lanemask_ge()
{
  return ~cuda_internal_var_lanemask_lt();
}

static inline uint32_t cuda_internal_var_lanemask_gt()
{
...
static inline uint32_t cuda_internal_var_total_smem_size()
{
...
static struct value *cuda_internal_var_api_failure_func_name()
{
  if (!cuda_get_last_driver_api_error_func_name_size())
    error(_("No error is available."));

  CORE_ADDR error_func_name_core_addr =
    cuda_get_symbol_address(_STRING_(CUDBG_REPORTED_DRIVER_API_ERROR_FUNC_NAME_ADDR));
  ...
  return value_from_pointer(builtin_type(cuda_get_gdbarch())->builtin_data_ptr, error_func_name_addr);
} 


/* Convenience variables */

static inline uint32_t cuda_internal_var_thread_lineno()
{
...
static inline bool cuda_internal_var_thread_active()
{
  const auto &c = cuda_current_focus::get().physical();
...
static inline uint32_t cuda_internal_var_focus_block_x()
{
  return cuda_current_focus::get().logical().blockIdx().x;
}

static inline uint32_t cuda_internal_var_focus_block_y()
{
  return cuda_current_focus::get().logical().blockIdx().y;
}

static inline uint32_t cuda_internal_var_focus_block_z()
{
  return cuda_current_focus::get().logical().blockIdx().z;
}

static inline uint32_t cuda_internal_var_focus_thread_x()
{
  return cuda_current_focus::get().logical().threadIdx().x;
}

static inline uint32_t cuda_internal_var_focus_thread_y()
{
  return cuda_current_focus::get().logical().threadIdx().y;
}

static inline uint32_t cuda_internal_var_focus_thread_z()
{
  return cuda_current_focus::get().logical().threadIdx().z;
}

static inline uint64_t cuda_internal_var_latest_kernelid()
{
  return cuda_state::last_launched_kernel_id();
}

static inline uint32_t cuda_internal_var_call_depth()
{
  const auto &c = cuda_current_focus::get().physical();
  return cuda_state::lane_get_call_depth(c.dev(), c.sm(), c.wp(), c.ln());
}

static inline uint32_t cuda_internal_var_num_regs_allocated()
{
  const auto &c = cuda_current_focus::get().physical();
  return cuda_state::warp_registers_allocated(c.dev(), c.sm(), c.wp());
}

static inline uint32_t cuda_internal_var_num_uregs()
{
  const auto &c = cuda_current_focus::get().physical();
  return cuda_state::device_get_num_uregisters(c.dev());
}

static inline std::vector<uint64_t> cuda_internal_present_kernel_ids()
{
...
static inline std::vector<std::vector<CuDim3>> cuda_internal_present_block_idxs()
{
  ...
  */
  ...
  std::vector<cuda_coord_set<cuda_coord_set_type::blocks, select_valid>> blocks_itr;
  cuda_coord_set<cuda_coord_set_type::kernels, select_valid> kernels_itr(cuda_coords::wild());
  
  for (const auto &kern : kernels_itr)
  {
    /* Create an iterator over every block in the kernel */
    cuda_coords filter( kern.physical().dev(), CUDA_WILDCARD, CUDA_WILDCARD, CUDA_WILDCARD,
                        kern.logical().kernelId(), CUDA_WILDCARD,
                        CUDA_WILDCARD_DIM, CUDA_WILDCARD_DIM, CUDA_WILDCARD_DIM, CUDA_WILDCARD_DIM);
...
template<typename T>
static typename std::enable_if<std::is_integral<T>::value, struct value *>::type convert_to_gdb_value(T arg)
{
  ...
  {
    return value_from_ulongest(builtin_type(arch)->builtin_unsigned_long_long, arg);
  }
}

template<typename T>
static struct value *convert_to_gdb_value(std::vector<T>& arg)
{
...
[[maybe_unused]]
static struct value *convert_to_gdb_value(struct value *arg)
{
  return arg;
}

static struct value *convert_to_gdb_value(CuDim3 arg)
{
  static type *dim3_type = lookup_struct("dim3", nullptr);

  return value_from_contents(dim3_type, reinterpret_cast<const gdb_byte *>(&arg));
}

static struct value *convert_to_gdb_value(bool arg)
{
  static type *bool_type = lookup_typename(current_language, "bool", 0, 0);

  return value_from_contents(bool_type, reinterpret_cast<const gdb_byte *>(&arg));
}

[[maybe_unused]]
static struct value *convert_to_gdb_value(const std::string& str)
{
  return value_cstring(str.c_str(), str.length(), builtin_type(cuda_get_gdbarch())->builtin_char);
}
...
template<typename T>
static cuda_internal_var_handler_fn wrap(const T& f)
{
  return [&]()
  {
    auto res = f();
...
/* === CUDA variable definition ===
  ...
  `handler` is the function to call to evaluate the variable.
      wrap() will attempt to convert the return value of the handler into a gdb `value`.
      If you're introducing an unsupported type, add an overload to `convert_to_gdb_value`.
  `host` indicates if the variable should be accessible in host code. Default requires CUDA focus + initialized API.
      Optional value, default to false.
  `aliases` is a list of alternative names for this handler.
      This is used for backward compatibility with old convenience variables and future uses should be avoided.
      Optional value, default to empty.
*/
static cuda_internal_var cuda_internal_variables[] =
{
  ...
  {"kernelid", wrap(cuda_internal_var_kernelid), false, {"cuda_focus_kernel_id"}},
  {"nkernelid", wrap(cuda_internal_var_nkernelid), true, {"cuda_num_total_kernels", "cuda_num_present_kernels"}},
  ...
  {"aggr_smem_size", wrap(cuda_internal_var_aggr_smem_size), false, {"cuda_shared_memory_size"}},
  {"dynamic_smem_size", wrap(cuda_internal_var_dynamic_smem_size)},

  /* Errors */
  {"cuda_api_failure_func_name", wrap(cuda_internal_var_api_failure_func_name), true},
  {"cuda_api_failure_return_code", wrap(cuda_get_last_driver_api_error_code), true},
  ...
  {"cuda_latest_launched_kernel_id", wrap(cuda_internal_var_latest_kernelid), true},
  {"cuda_call_depth", wrap(cuda_internal_var_call_depth)},
  {"cuda_num_registers_allocated", wrap(cuda_internal_var_num_regs_allocated)},
...
static struct value *cuda_internal_var_wrapper_host(struct gdbarch *arch, struct internalvar *var, void *data)
{
...
static struct value *cuda_internal_var_wrapper(struct gdbarch *arch, struct internalvar *var, void *data)
{
...
_initialize_cuda_internal_vars ()
{
  ...
  {
    const internalvar_funcs *wrapper = v.is_host ? &cuda_internal_var_funcs_host : &cuda_internal_var_funcs;
