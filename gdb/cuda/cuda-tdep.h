...
const char *cuda_address_class_to_name (ptxStorageKind address_class);

// Read from segmented address using the address class.
// Returns 0 if the read was successful, 1 if the read failed.
// On failure, if there should be a fallback to host access, set hostaddr to
// the host address.
int cuda_read_memory_with_address_class (CORE_ADDR segmented_address,
					 int address_class, gdb_byte *buf,
					 int len, uint64_t &hostaddr);

// Write to segmented address using the address class.
// Returns 0 if the write was successful, 1 if the write failed.
// On failure, if there should be a fallback to host access, set hostaddr to
// the host address.
int cuda_write_memory_with_address_class (CORE_ADDR segmented_address,
					  int address_class,
					  const gdb_byte *buf, int len,
					  uint64_t &hostaddr);

// Takes a possibly tagged core address and reads from the appropriate segment.
// Returns 0 if the read was successful, 1 if the read failed.
int cuda_read_memory (gdbarch *gdbarch, CORE_ADDR address, gdb_byte *buf,
		      int len);

// Read from core address using the address class to determine the segmented
// address. Returns 0 if the read was successful, 1 if the read failed.
int cuda_read_memory (gdbarch *gdbarch, CORE_ADDR address, int address_class,
		      gdb_byte *buf, int len);

// Read from core address using the type instance flags to determine the
// address class. Returns 0 if the read was successful, 1 if the read failed.
int cuda_read_memory (gdbarch *gdbarch, CORE_ADDR address,
		      type_instance_flags flags, gdb_byte *buf, int len);

// Read into the value using the type to determine the address class.
// Returns 0 if the read was successful, 1 if the read failed.
...
// Write to segmented address using the address class.
// Returns 0 if the write was successful, 1 if the write failed.
int cuda_write_memory (gdbarch *gdbarch, CORE_ADDR segmented_address,
		       int address_class, const gdb_byte *buf, int len);

// Write to core address using the type instance flags to determine the
// address class. Returns 0 if the write was successful, 1 if the write failed.
int cuda_write_memory (gdbarch *gdbarch, CORE_ADDR address,
		       type_instance_flags flags, const gdb_byte *buf,
		       int len);
