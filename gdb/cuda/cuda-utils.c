...
cuda_ptx_cache_find_element (struct frame_id frame_id, int dwarf_regnum)
{
  ...
  for (auto it = cuda_ptx_register_cache.begin ();
    ...
    if (coords == it->coords && frame_id_eq (frame_id, it->frame_id)
 ...
 */
void
cuda_ptx_cache_store_register (struct frame_info *frame, int dwarf_regnum,
                               struct value *value)
{
...
struct value *
cuda_ptx_cache_get_register (struct frame_info *frame, int dwarf_regnum,
                             struct type *type)
{
  struct value *retval;
