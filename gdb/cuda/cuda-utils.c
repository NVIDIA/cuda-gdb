...
void cuda_nat_bypass_signals_cleanup (unsigned char *sigs)
{
...
cuda_ptx_cache_find_element (struct frame_id frame_id, int dwarf_regnum)
{
  ...
  if (cuda_coords_get_current (&coords)) return cuda_ptx_register_cache.end ();

  for (auto it = cuda_ptx_register_cache.begin (); it != cuda_ptx_register_cache.end (); ++it)
    if (cuda_coords_equal (&coords, &it->coords) &&
	frame_id_eq (frame_id, it->frame_id) &&
	it->dwarf_regnum == dwarf_regnum) return it;
...
cuda_ptx_cache_get_register (struct frame_info *frame, int dwarf_regnum, struct type *type)
{
  ...
  if (elem == cuda_ptx_register_cache.end () || elem->len != TYPE_LENGTH(type) ||
...
cuda_ptx_cache_update_local_vars (void)
{
  ...
  if (!cuda_focus_is_device() || find_thread_ptid (current_inferior ()->process_target (), inferior_ptid)->executing ()) return;
  cb_data.m_frame = get_current_frame();
  if (!cb_data.m_frame) return;
...
cuda_ptx_cache_refresh (void)
{
  ...
  if (!cuda_ptx_register_cache.empty ())
    {
...
cuda_managed_address_p (CORE_ADDR addr)
{
  ...
  for (auto &elem : cuda_managed_memory_regions)
    if (elem.begin <= addr && elem.end > addr) return true;
  return false;
}
...
cuda_is_value_managed_pointer (struct value *value)
{
  ...
  try
    {
      result = cuda_managed_address_p (unpack_pointer (type, value_contents_for_printing (value).data ()));
    }
