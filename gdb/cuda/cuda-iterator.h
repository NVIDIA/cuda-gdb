    ...
    for (uint32_t dev = 0; dev < cuda_state::get_num_devices (); ++dev)
      {
		...
		const bool validWarp = cuda_state::sm_valid (dev, sm)
		  && cuda_state::warp_valid (dev, sm, wp);
                ...
                if (atClock
                    && cuda_state::warp_timestamp_valid (dev, sm, wp)
                    && (cuda_state::warp_timestamp (dev, sm, wp) < cuda_clock ()))
                  continue;
			...
			&& cuda_state::lane_timestamp_valid (dev, sm, wp, ln)
                        && (cuda_state::lane_timestamp (dev, sm, wp, ln) < cuda_clock ()))
                      continue;
			  ...
			  && cuda_state::warp_valid (dev, sm, wp)
			  && cuda_state::lane_valid (dev, sm, wp, ln))
			 ? cuda_state::lane_get_thread_idx (dev, sm, wp, ln)
			 : CUDA_INVALID_DIM);
