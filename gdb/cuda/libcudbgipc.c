...
/*Globals */
CUDBGNotifyNewEventCallback41 cudbgDebugClientCallback = NULL;
CUDBGIPC_t commOut;
CUDBGIPC_t commIn;
CUDBGIPC_t commCB;
...
cudbgCallbackHandler (void *arg)
{
  CUDBGCBMSG_t data;
  CUDBGResult res;
  CUDBGEventCallbackData41 cbData;
  sigset_t sigset;
  ...
  for (;;)
	...
	{
	  cudbgipc_trace (
	      "failure while waiting for callback data! (res = %d)", res);
	  break;
	}
      if (data.terminate)
	{
	  cudbgipc_trace (
	      "Callback handler thread received termination data.\n");
	  break;
	}
...
cudbgipcCreate (CUDBGIPC_t *ipc, int from, int to, int flags)
{
  ...
  else if (mkfifo (ipc->name, S_IRGRP | S_IWGRP | S_IRUSR | S_IWUSR)
	   && errno != EEXIST)
    {
      cudbgipc_trace (
	  "Failed to create fifo (from=%u, to=%u, file=%s, errno=%d)", from,
	  to, ipc->name, errno);
      return CUDBG_ERROR_COMMUNICATION_FAILURE;
    }

    /* If cuda-gdb is launched as root, make pipes are writeable by UID of
     * debugged process */
#ifndef GDBSERVER
  if (getuid () == 0)
    {
	...
	{
	  cudbgipc_trace (
	      "Changing ownership to pid %d uid failed for %s, errno=%d", pid,
	      ipc->name, errno);
	  return CUDBG_ERROR_COMMUNICATION_FAILURE;
	}
    }
  ...
  do
	...
	{
	  cudbgipc_trace ("Pipe opening failure (from=%u, to=%u, flags=%x, "
			  "file=%s, errno=%d)",
  ...
  if (timeout_in_seconds < 0)
    {
      cudbgipc_trace (
	  "Pipe opening timeout (from=%u, to=%u, flags=%x, file=%s, errno=%d)",
	  ipc->from, ipc->to, flags, ipc->name, errno);
      return CUDBG_ERROR_COMMUNICATION_FAILURE;
    }
  ...
  if ((flags & O_WRONLY) == O_WRONLY)
	...
	{
	  cudbgipc_trace (
	      "Cannot unlink fifo (from=%u, to=%u, file=%s, errno=%d)",
...
cudbgipcInitializeCommIn (void)
{
  ...
  res = cudbgipcCreate (&commIn, LIBCUDBG_PIPE_ENDPOINT_RPCD,
			LIBCUDBG_PIPE_ENDPOINT_DEBUG_CLIENT,
#if __QNXHOST__
			/* On QNX readonly pipes always return EOF when there
			   is no data so create a fake writer too */
			O_RDWR | O_NONBLOCK
#else
			O_RDONLY | O_NONBLOCK
#endif
  ...
  cudbgipc_trace ("initialized commIn (from = %d, to = %d)", commIn.from,
		  commIn.to);
  return CUDBG_SUCCESS;
}
...
cudbgipcInitializeCommOut (void)
{
  ...
  res = cudbgipcCreate (&commOut, LIBCUDBG_PIPE_ENDPOINT_DEBUG_CLIENT,
			LIBCUDBG_PIPE_ENDPOINT_RPCD, O_WRONLY);
  if (res != CUDBG_SUCCESS)
    return res;

  cudbgipc_trace ("initialized commOut (from = %d, to = %d)", commOut.from,
		  commOut.to);
  return CUDBG_SUCCESS;
}
...
cudbgipcInitializeCommCB (void)
{
  ...
  res = cudbgipcCreate (&commCB, LIBCUDBG_PIPE_ENDPOINT_RPCD_CB,
  ...
  cudbgipc_trace ("initialized commCB (from = %d, to = %d)", commCB.from,
		  commCB.to);
 ...
 * Following RSP commands are used to proxy CUDADBG API from cuda-gdb to
 * cuda-gdbserver:
...
public:
  ...
  send_request (void)
  {
    ...
    gdb::array_view<const char> view (m_out_buffer.data (),
				      m_out_buffer.size ());
  ...
  received (gdb::array_view<const char> &buf) override
  {
    ...
    m_processing_multi_packet
	= (memcmp (buf.data (), "MP;", strlen ("MP;")) == 0);
    ...
    else if (m_processing_multi_packet
	     || memcmp (buf.data (), "OK;", strlen ("OK;")) == 0)
      {
	...
	if (m_processing_multi_packet)
	  {
	    ...
	    m_out_buffer.clear ();
	    m_out_buffer.insert (m_out_buffer.begin (), more_data.begin (),
				 more_data.end ());
	    /* Send it */
	    send_request ();
	  }
...
cudbgipcPush (CUDBGIPC_t *out)
{
  ...
  /* Push out the data */
  for (offset = 0, writeCount = 0; offset < out->dataSize;
       offset += writeCount)
    {
      writeCount = write (out->fd, buf + offset, out->dataSize - offset);
      if (writeCount < 0)
	{
	  /* Forward SIGINT received during syscall to main thread signal
	   * handler */
	  ...
	  if (errno != EAGAIN && errno != EINTR)
	    {
	      cudbgipc_trace ("Fifo write error (from=%u, to=%u, "
			      "out->dataSize=%zu, offset=%zu, errno=%d)",
			      out->from, out->to, out->dataSize, offset,
			      errno);
	      return CUDBG_ERROR_COMMUNICATION_FAILURE;
	    }
	  writeCount = 0;
...
cudbgipcRead (CUDBGIPC_t *in, void *buf, size_t size)
{
  ...
  for (offset = 0; offset < size; offset += readCount)
      ...
      if (readCount < 0)
	{
	  ...
	  if (errno != EAGAIN && errno != EINTR)
	    {
	      cudbgipc_trace ("Fifo read error (from=%u, to=%u, size=%zu, "
			      "offset=%zu, errno=%d)",
...
cudbgipcWait (CUDBGIPC_t *in)
{
  ...
  do
    {
	  ...
	  && pthread_self () != cudagdbMainThreadHandle)
	pthread_kill (cudagdbMainThreadHandle, SIGINT);
    }
  ...
  if (ret == -1)
    {
      cudbgipc_trace ("Select error (from=%u, to=%u, errno=%u)", in->from,
		      in->to, errno);
      return CUDBG_ERROR_COMMUNICATION_FAILURE;
    }
  ...
  if (FD_ISSET (in->fd, &errFDS))
    {
      cudbgipc_trace ("Select error on in->fd (from=%u, to=%u, errno=%u)",
		      in->from, in->to, errno);
      return CUDBG_ERROR_COMMUNICATION_FAILURE;
    }
